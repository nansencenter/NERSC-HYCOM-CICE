...
! SVN $URL: https://svn-ccsm-models.cgd.ucar.edu/csm_share/trunk_tags/share3_121022/shr/shr_orb_mod.F90 $
!===============================================================================
...
!echmod
!echmod   use shr_kind_mod
!echmod   use shr_sys_mod
!echmod   use shr_const_mod
!echmod   use shr_log_mod, only: s_loglev  => shr_log_Level
!echmod   use shr_log_mod, only: s_logunit => shr_log_Unit
   use ice_kinds_mod, only: dbl_kind, int_kind !echmod
   use ice_exit, only: abort_ice               !echmod
   use ice_constants, only: pi                 !echmod
   use ice_fileunits, only: ice_stdout         !echmod

   IMPLICIT none

   integer (kind=int_kind), parameter :: SHR_KIND_R8 = dbl_kind     !echmod
   integer (kind=int_kind), parameter :: SHR_KIND_IN = int_kind     !echmod
   integer (kind=int_kind), parameter :: s_logunit   = ice_stdout   !echmod
   integer (kind=int_kind), parameter :: s_loglev    = 0            !echmod
...
!echmod         call shr_sys_abort(subname//' ERROR: unreasonable obliq')
         call abort_ice('unreasonable oblip') !echmod
...
!echmod         call shr_sys_abort(subname//' ERROR: unreasonable obliq')
         call abort_ice('unreasonable obliq') !echmod
      end if
      if( (eccen < SHR_ORB_ECCEN_MIN).or.(eccen > SHR_ORB_ECCEN_MAX) ) then
         write(s_logunit,F03) 'Input eccentricity unreasonable: ', eccen
!echmod         call shr_sys_abort(subname//' ERROR: unreasonable eccen')
         call abort_ice('unreasonable eccen') !echmod
      end if
      if( (mvelp < SHR_ORB_MVELP_MIN).or.(mvelp > SHR_ORB_MVELP_MAX) ) then
         write(s_logunit,F03) 'Input mvelp unreasonable: ' , mvelp
!echmod         call shr_sys_abort(subname//' ERROR: unreasonable mvelp')
         call abort_ice('unreasonable mvelp') !echmod
      end if
      eccen2 = eccen*eccen
      eccen3 = eccen2*eccen
...
!echmod         call shr_sys_abort(subname//' ERROR: unreasonable year')
         call abort_ice('unreasonable year') !echmod
      end if
