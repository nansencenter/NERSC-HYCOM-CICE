      ...
      do iblk=1,nblocks
         ...
         elseif (trim(expFieldName(ifld)) == "sitx") then
            ...
            do i=ilo,ihi
               ig  = this_block%i_glob(i)
               jg  = this_block%j_glob(j)
               expData(ifld,iblk)%p(ig,jg) = strocnxT(i,j,iblk)*aice(i,j,iblk) ! NB: Check T to U and sign
            end do
            end do
         ! Ice-ocean stress
         elseif (trim(expFieldName(ifld)) == "sity") then
            ...
            do i=ilo,ihi
               ig  = this_block%i_glob(i)
               jg  = this_block%j_glob(j)
               expData(ifld,iblk)%p(ig,jg) = strocnyT(i,j,iblk)*aice(i,j,iblk) ! NB: Check T to U and sign
            end do
            end do
