...
It means the model needs the nesting files named by archm.yyyy_ddd_12.a[b] for '-1', otherwise, it would be archv.yyyy_ddd_00.a[b]. For the first implementation, the nesting boundary files are derived from daily average. The second implementation of 'bnstfq=1' provides a clearer snapshot. For the TP5 implementation, the nesting files are interpolated by the daily source, so the recommendation is to use the setting of '-1' within the nesting boundary.
