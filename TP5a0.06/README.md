...
It means the model needs the nesting files named by archm.yyyy_ddd_12.a[b] for '-1', otherwise, it would be archv.yyyy_ddd_00.a[b]. For the first implementation, the daily nesting boundary will be fed into the model four times (if the input time step is every 6 hours). To the second implimentation of 'bnstfq=1', the boundary will be readed by once time for each day. For the TP5 implementation, the nesting files are interpolated by the daily source, so the recommendation is to use the setting of '-1' within the nesting boudary.
