...
usage="
This script is a wrapper around ensstat_field to create 3D version
of field correlations between 3D variable field1 and variable field2
at layer index k2. It calls ensstat_field several times to produce 
the 3D field. See ensstat_field for more info on that routine. 
...
usage :
   $(basename $0) field1 field2 k2 hycomfiles
Example:
   $(basename $0) saln saln 1 hycomfiles
Will create a 3D netcdf file with correlation of salinity in layer
1 with salinity in all layers
...
ENSSTAT=$(dirname $0)/ensstat_field
...
for k in $(seq 1 $kdm) ; do
   ...
   # Rune ensstat_field
   ck=$(printf "%2.2d" $k)
   $ENSSTAT $field1 $k $field2 $kind2 $@

   # move to temporary file
   mv ensstat_field.nc ensstat_field$ck.nc 
   ...
   ncrename -v ave_a${field1}$ck,ave_a${field1} ensstat_field$ck.nc  
   ncrename -v var_a${field1}$ck,var_a${field1} ensstat_field$ck.nc 
   ...
   ensstat_field$ck.nc 
   flist="$flist ensstat_field$ck.nc"
done
ncecat -O -u layer -v corr_a${field1}_b${field2}$ckind2,cov_a${field1}_b${field2}$ckind2,\
ave_a${field1},var_a${field1} $flist ensstat_field3D.nc

rm $flist 
echo 
echo "3D fields in ensstat_field3D.nc"
