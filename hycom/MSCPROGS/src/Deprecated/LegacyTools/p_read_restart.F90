program read_restart_mem (rungen,rt,imem1,imem2,memproc)
   use mod_common
   use mod_hycomfuncs
   use mod_year_info
   use mod_states
   ...
   ! Global arrays
   real, dimension(itdm,jtdm) :: tmp,tmpmix,tpbot,tpsikk,tthkk,tdpmixl
   ...
   do imem=imem1,imem2
      ...
      elseif (rident == cident2) then
         ...
         call xcaput (tpbot  ,pbot  ,0)
         call xcaput (tpsikk ,psikk ,0)
         call xcaput (tthkk  ,thkk  ,0)
         call xcaput (tdpmixl,dpmixl,0)
      ...
      ! Distribute state variable among tiles
      call states_to_micom()

      ! Write the distributed values into ensemble
      call write_ensemble('HYCOM',imem)
