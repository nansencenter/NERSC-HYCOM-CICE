module m_read_ecmwf
contains
subroutine read_ecmwf(rt,plon,plat,depths,rforce)
   use mod_xc
...
! --- Open input files
   ...
   if (.not.ex) then
      if (mnproc==1) print *,'read_ecmwf:  file does not exist :',trim(path0),'ecmwf_',rt%cyy,'_tem.uf'
      call xcstop ('(read_ecmwf)')
      stop
   endif
   ...
   if (.not.ex) then
      if (mnproc==1) print *,'read_ecmwf:  file does not exist :',trim(path0),'ecmwf_',rt%cyy,'_dew.uf'
      call xcstop ('(read_ecmwf)')
      stop
   endif
   ...
   if (.not.ex) then
      if (mnproc==1) print *,'read_ecmwf:  file does not exist :',trim(path0),'ecmwf_',rt%cyy,'_msl.uf'
      call xcstop ('(read_ecmwf)')
      stop
   endif
   ...
   if (.not.ex) then
      if (mnproc==1) print *,'read_ecmwf:  file does not exist :',trim(path0),'ecmwf_',rt%cyy,'_uuu.uf'
      call xcstop ('(read_ecmwf)')
      stop
   endif
   ...
   if (.not.ex) then
      if (mnproc==1) print *,'read_ecmwf:  file does not exist :',trim(path0),'ecmwf_',rt%cyy,'_vvv.uf'
      call xcstop ('(read_ecmwf)')
      stop
   endif
   ...
   if (.not.ex) then
      if (mnproc==1) print *,'read_ecmwf:  file does not exist :',trim(path0),'ecmwf_',rt%cyy,'.hdr'
      call xcstop ('(read_ecmwf)')
      stop
   endif
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! sea level pressure
   call bilin_ecmwf2(msl,nxd,nyd,lonref,latref,dlon,dlat,synslp,mlon,plat,depths)

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! air temperature
   call bilin_ecmwf2(tem,nxd,nyd,lonref,latref,dlon,dlat,synairtmp,mlon,plat,depths)

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! windstresses and windspeed
   call bilin_ecmwf2(uuu,nxd,nyd,lonref,latref,dlon,dlat,synuwind,mlon,plat,depths)
   call bilin_ecmwf2(vvv,nxd,nyd,lonref,latref,dlon,dlat,synvwind,mlon,plat,depths)
   !print *,'read_ecmwf --- check rotate for tiled grid'
...
! relative humidity
   ...
   enddo
   !$OMP END PARALLEL DO
   call bilin_ecmwf2(rhum,nxd,nyd,lonref,latref,dlon,dlat,synrelhum(:,:),mlon,plat,depths)
