...
program trip_riverpaths
   ...
   integer, dimension(nx,ny) :: direction, destinationx, destinationy,lmask
   ...
   inquire(exist=ex,file=trim(path0)//tfile)
   if (.not. ex) then 
      print *,'TRIP database not found in '//trim(path0)
   ...
   open(10,file=trim(path0)//tfile,status='old',action='read',iostat=ios)
   do j=1,ny
      read(10,*,iostat=ios) txtline

      if (ios/=0) then
         print *,'error readint trip file '//trim(path0)//tfile
         stop
      end if
   ...
   close(10)
   !print *,maxval(direction)
   !print *,minval(direction)



   !where (lmask==0) direction=9

   !call exit(1)

! For dat file
!   open(10,file='Data/'//tfile,status='old',action='read',iostat=ios)
!   ios=0
!   do while(ios==0)
!      read(10,*,iostat=ios) tmplon,tmplat,tmpdir
!      if (ios==0) then
!         i=nint(tmplon/dx - 1./2.)+1
!         j=nint(tmplat/dy +90./dy - 1./2.)+1
!         direction(i,j)=tmpdir
!         !print *,tmplon,lon(i),tmplat,lat(j)
!      end if
!   end do
!   close(10)
