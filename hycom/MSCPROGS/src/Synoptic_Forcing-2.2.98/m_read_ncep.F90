...
contains
   ...
   subroutine read_ncep(rt,plon,plat,depths)
      ...
      real :: rfac,wndfac,cd_new,w4,wfact,dlon,dlat,flon,flat
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! NCEP land mask
      if (ldump) write(lp, *) 'Using NCEP land mask'
      call ncvar_read(flmsk,vlmsk,ncep_lmsk   ,nlon,nlat, 1,1,1)

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! NCEP geopot height
      call ncvar_read(fhgt,vhgt,ncep_hgt   ,nlon,nlat, 1,1,1)
      if (ldump) write(lp, *) 'Using NCEP geopot height (orography pressure correction'
...
! NCEP ice concentration
      ...
      call interpug(ncep_icec(:,:,1),nlon,nlat,flon,flat,dlon,dlat,&
       tmp_icec(:,:),mlon,plat,itype,lat)
...
! NCEP precipitation   kg/(m^2 s) -> m/month (rho=1000)
      if (lsynprecip) then
         ...
         call interpug(nceptmp(:,:,1),nlon,nlat,flon,flat,dlon,dlat,&
               synprecip,mlon,plat,itype,lat)
...
! NCEP winds
      if (lsynwnd) then
         ...
         call interpug(nceptmp(:,:,1),nlon,nlat,flon,flat,dlon,dlat,&
               synuwind,mlon,plat,itype,lat)
         call ncvar_read(fvwnd,vvwnd,nceptmp,nlon,nlat,1,irec,irec)
         call interpug(nceptmp(:,:,1),nlon,nlat,flon,flat,dlon,dlat,&
               synvwind,mlon,plat,itype,lat)

         ! Rotate winds to model grid 
         if (wndflag==1) then
         ! --- Rotate winds/stress to model u-points
...
!NCEP relative humidity and pressure
      if (lsynslp .or. lsynrelhum) then
         if (ldump) write(lp,'(a)') ' Using NCEP synoptic SLP + SHUM'
         ...
         do ix=1,idm
            ...
            satvapp=satvap(synairtmp(ix,jy))

            ! Vapor mixing ration (synrelhum is shum at this point)
            synvapmix(ix,jy)=synrelhum(ix,jy) / (1.-synrelhum(ix,jy))
            ...
            ! Relative humidty
            synrelhum(ix,jy)=spechum_to_relhum(synrelhum(ix,jy),sathumid)
            synairtmp(ix,jy)=synairtmp(ix,jy)
         end do
         end do
      end if
...
!NCEP downwelling shortwave radiation
      if (lsynshwflx ) then
         if (ldump) write(lp,'(a)') ' Using NCEP synoptic DSWRF '
         ...
         nceptmp=nceptmp*.5
         call interpug(nceptmp(:,:,1),nlon,nlat,flon,flat,dlon,dlat,&
               synshwflx,mlon,plat,itype,lat)
      end if
#if defined (SOUP)
         print *,'Windspd before soup',synwndspd(150,150)
            call read_soup(rt,plon,plat,synuwind,synvwind,&
                 synwndspd,syntaux,syntauy)
         print *,'Windspd after soup',synwndspd(150,150)
#endif
