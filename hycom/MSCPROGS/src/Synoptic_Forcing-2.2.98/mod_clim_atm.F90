...
real, allocatable, dimension(:,:,:)  :: &
   ...
   clmradflx, clmshwflx, clmslp, clmustar, &
   clm_Qraddwn, clm_Qlwdwn, clm_Tsfc
...
contains
   ...
   allocate(clmustar (idm,jdm,2))
   allocate(clm_Qraddwn (idm,jdm,2))
   allocate(clm_Qlwdwn  (idm,jdm,2))
   allocate(clm_Tsfc    (idm,jdm,2))
   clmuwind=0.
   clmvwind=0.
   clmwndspd=0.
   clmtaux=0.
...
if (clmflag=='era40') then
   ...
   call interpug(tmpdata,nlon,nlat,flon,flat,dlon,dlat, &
                          field,mlon,plat,itype)   
...
elseif (clmflag=='ncepr') then
   ...
   flon=tmplon(1) ; flat=tmplat(1) ;
   dlon=tmplon(2)-tmplon(1) ; dlat=tmplat(2)-tmplat(1)


   mlon = plon ; where (mlon < 0.) mlon=mlon+360.
   call interpug (tmpdata,nlon,nlat,flon,flat,dlon,dlat,field,mlon,plat,1,tmplat)
...
!$OMP END PARALLEL DO
   ...
   do i=1,idm
      ...
      ! Calculate Radiative flux (positive down)
      ! Radiation flux is Qlwdn (downwelling longwave). Conversion to net LW is done in hycom
      if (lwflag .eq. -1) then
         if (have_Qlwdwn_clim) then
            clmradflx(i,j,islot)= clm_Qlwdwn(i,j,islot)
         else 
            clmradflx(i,j,islot)= (clmairtmp(i,j,islot))**4 
            clmradflx(i,j,islot)= clmradflx(i,j,islot)*stefanb*0.95
         end if
      ! Radiation flux is Qsw + net Qlw, sst based on climatology. HYCOM does
      ! correction for surface sst in code
      elseif (lwflag .ge. 0) then
         clmradflx(i,j,islot)= (clmairtmp(i,j,islot))**4    - &
                               (clmsst   (i,j,islot)+t0deg)**4
         clmradflx(i,j,islot)=clmradflx(i,j,islot)*stefanb*0.95
         clmradflx(i,j,islot)=clmradflx(i,j,islot) &
                             +clmshwflx(i,j,islot)
         if (have_Qraddwn_clim.and.lwflag.eq.0) then
            clmradflx(i,j,islot)= clm_Qraddwn(i,j,islot)
         end if
      else
         write(lp,'(a)') "ERROR:lwflag>=2 not supported"
         call exit(1)
      end if

      ! KAL - estimate ustar from taux, tauy
      clmustar(i,j,islot) = sqrt((clmtaux(i,j,islot)**2 + clmtauy(i,j,islot))/1000.)
