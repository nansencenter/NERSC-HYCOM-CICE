...
!      undefined
   ...
   integer, save :: lwflag            ! lwflag from blkdat.input
   integer, save :: ustflg            ! ustflag from blkdat.input
   integer, save :: albflg            ! 
...
! Reconfigurable  paths
   ...
   real, allocatable, dimension(:,:)  ::                synuwind, &
     ...
     synradflx, synshwflx, synslp, synssr, cawdir, synustar,&
     Qlwdwn, Qraddwn, syntsfc
...
contains
   ...
   allocate(Qlwdwn   (idm,jdm))
   allocate(Qraddwn  (idm,jdm))
   allocate(syntsfc  (idm,jdm))
   synuwind (:,:)=0.
   synvwind (:,:)=0.
   synustar (:,:)=0.
   synwndspd(:,:)=0.
   ...
   ! Time info and forcing info from blkdat.input. 
   call parse_blkdat('yrflag',yrflag)
   call parse_blkdat('wndflg',wndflag)
   call parse_blkdat('lwflag',lwflag)
   call parse_blkdat('albflg',albflg)
   call parse_blkdat('ustflg',ustflg)

   ! Wind/ustar checks
   if (ustflg==3 .and. .not.lsynustar) then
      write(lp,'(a)') "ERROR: ustar fields to hycom needed, but not available/not implemented for this setup"
      call exit(1)
   end if
   if (wndflag==1) then
      write(lp,'(a)') "ERROR: Wind flag 'wndflg' set to 1 is not recommended. Change setting in blkdat.input"
      call exit(1)
   end if

   ! Consistency checks
   ! lw flag checks and warnings
   if (lwflag==-1) then
      write(lp,'(a)') "radflx field is downwelling longwave radiaition"
   elseif (lwflag==0) then
      write(lp,'(a)') "radflx field is Qsw+Qlw, no correction"
   elseif (lwflag==1) then
      write(lp,'(a)') "radflx field is Qsw+Qlw, climatology correction"
   elseif (lwflag==2) then
      write(lp,'(a)') "radflx field is Qsw+Qlw, air surface temp correction"
   else 
      write(lp,'(a)') "ERROR: lwflag must be -1,0,1, or2"
      call exit(1)
   end if
   if (lwflag==-1.and. .not.have_Qlwdwn) then
      write(lp,'(a)') "WARNING: Downwelling longwave is based on air temperature, not flux fields"
   end if
   if (lwflag==-1.and. .not.have_Qlwdwn_clim) then
      write(lp,'(a)') "WARNING: Climatology downwelling longwave is based on air temperature, not flux fields"
   end if
   if (lwflag.gt.0.and. .not.have_Qraddwn) then
      write(lp,'(a)') "WARNING: Net downward radiation is based on air temperature, not flux fields"
   end if
   if (lwflag.gt.0.and. .not.have_Qraddwn_clim) then
      write(lp,'(a)') "WARNING: Climatology Net downward radiation is based on air temperature, not flux fields"
   end if

   ! albflg consistency check
   if (albflg==0) then
      write(lp,'(a)') "Qsw is Qswdwn*(1.-albedo)"
      write(lp,'(a)') "ERROR: albflg must be 1 or 2, albflg = 0 is not supported"
      call exit(1)
   elseif (albflg==1 .or. albflg==2) then
      write(lp,'(a)') "Qsw is Qswdwn"
   else 
      write(lp,'(a)') "ERROR: albflg must be 1 or 2"
      call exit(1)
   end if


   if (albflg.gt.0 .and. lwflag.ne.-1) then
      write(lp,'(/ a /)')  'error - albflg>0 requires lwflag=-1'
      call flush(lp)
      call exit(1)
   endif
