...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!Scan through records in SEDIMENT input file - First pass fills water column where
!local model is shallower than the global model

   ! Open global restart file
   call zaiopf_global(trim(grestart)//'SED.a','old',gnop)
   open(gnop,file=trim(grestart)//'SED.b',status='old')

   ! Open temporary restart file
   call zaiopf_local('pass1SED.a','replace',lnop)
   open(lnop,file='pass1SED.b',status='replace')
   ...
   do while (ios==0) 
      ...
      if (ios==0) then
         ...
         cfld=a80(1:8)
         print*, cfld
         read(a80(indx+1:len_trim(a80)),*) itmp,rtmp,lcoord,rtmp,bxmin,bxmax
         if (lcoord>kdm .or. istep>3) then
            print *
            print *,a80
            print *,'global coord exceed those in blkdat!'
         ...
         call zaiowr_local(lfld,ldummy,.false.,axmin,axmax,lnop,.true.)
         write(lnop,4101) cfld,itmp,rtmp,lcoord,rtmp,axmin,axmax
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! Second pass fills for SEDIMENT, the  water column where global model is DEEPER than local model
! (extends deepest layer)

   ! Open local file
   call zaiopf_local('pass1SED.a','old',lnop)
   open(lnop,file='pass1SED.b',status='old')
   lrestart='curviint.'//trim(grestart)
   call zaiopf_local(trim(lrestart)//'SED.a','replace',lnop2)
   open(lnop2,file=trim(lrestart)//'SED.b',status='replace')
   ...
   do while (ios==0) 
      ...
      if (ios==0) then
         ...
         call zaiowr_local(lfld,ldummy,.false.,axmin,axmax,lnop2,.true.)
         write(lnop2,4101) cfld,itmp,rtmp,lcoord,rtmp,axmin,axmax
!         write(lnop2,4100) cfld,lcoord,istep,axmin,axmax
      end if
   end do
   call zaiocl_local (lnop)
   ...
   if (trim(gicefname)/='') then
      ...
      if (ex) then 
         ...
         call bilinear_calc('ticem',lticem,loldfld,lmsk,gticem, &
                             dp(:,:,1,1),lcoord)
         call bilinear_calc('tsrfm',lticem,loldfld,lmsk,gtsrfm, &
                             dp(:,:,1,1),lcoord)
...
4101  format(a,' = ',i7,f8.2 i3, f6.3,1p2e16.7)
