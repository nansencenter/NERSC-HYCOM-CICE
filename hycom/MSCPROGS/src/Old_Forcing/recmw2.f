      SUBROUTINE recmw2(dd,dlat,dlon,nxd,nyd,
     +               filen,a,b,vmin,vmax)
c -----------------------------------------------------------------
...
c ----------------------------------------------------------------
     ...
     +     dlat(nyd),dlon(nxd)
      ...
      INTEGER x,y,k,mo ,id(320)

      IF(nxd.NE.320)STOP 'Check  recmw2'
      ...
      DO y=1,nyd                     !Calc. lattitude
        dlat(y)= -89.142 +(y-1)*1.121283
      ENDDO
      DO x=1,nxd                     !Calc. longitude.
        dlon(x) = FLOAT(x-161)*1.125
      ENDDO
  600 FORMAT(20I4)
      RETURN
      END
