...
! 2nd order method
   ...
   real, dimension(:,:), allocatable :: plon,plat,depths, &
      scpx, scpy
   integer, dimension(:,:), allocatable :: ipmask
   ...
   INTEGER*8 :: mxCreateNumericMatrix, mxGetPr, mxClassIDFromClassName, matopen,  &
...
!-------------------------------------------------------------------
   ...
   elseif (iargc()==0) then
      ...
      print *,'icedrift2:five or zero args' 
      print *,'Usage - 5 args:'
...
!-------------------------------------------------------------------
   ...
   lgth = len_trim(flnmdep)
   call zaiopf(flnmdep(1:lgth)//'.a', 'old', 701)
   open (unit=701,file=flnmdep(1:lgth)//'.b',&
         status='old')

   call zaiord(depths,ipmask,.false.,hmin,hmax,701)
   ...
   lgth = len_trim(flnmgrd)
   call zaiopf(flnmgrd(1:lgth)//'.a', 'old', 701)
   open (unit=701,file=flnmgrd(1:lgth)//'.b', &
         status='old')
   read(701,*) i
   read(701,*) j
   read(701,*) 
   if (idm/=i) then
      write(lp,'(a,i5,a,i5)') 'Expected idm =',idm, &
            ' (from blkdat.input) but '//flnmgrd(1:lgth)// &
            ' reads idm = ',i
      print *,'(icedrift2:regional.grid dimensions)'
      call exit(1)
   else if (jdm/=j) then
      write(lp,'(a,i5,a,i5)') 'Expected jdm =',jdm, &
            ' (from blkdat.input) but '//flnmgrd(1:lgth)// &
            ' reads jdm = ',j
      print *,'(icedrift2:regional.grid dimensions)'
      call exit(1)
   ...
   call zaiord(plon,ipmask,.false.,hmin,hmax,701)
   read(701,'(a80)') cline
   i = index(cline,'=')
   read (cline(i+1:),*) hminb,hmaxb
   if (abs(hmin-hminb).gt.abs(hminb)*1.e-4 .or. &
   ...
   call zaiord(plat,ipmask,.false.,hmin,hmax,701)
   read(701,'(a80)') cline
   i = index(cline,'=')
   read (cline(i+1:),*) hminb,hmaxb
   if (abs(hmin-hminb).gt.abs(hminb)*1.e-4 .or. &
   ...
   call zaiord(scpx,ipmask,.false.,hmin,hmax,701)
   read(701,'(a80)') cline
   i = index(cline,'=')
   read (cline(i+1:),*) hminb,hmaxb
   if (abs(hmin-hminb).gt.abs(hminb)*1.e-4 .or. &
       ...
       write(lp,'(/ a / a,1p3e14.6 / a,1p3e14.6 /)') &
         ...
         '.a,.b min = ',hmin,hminb,hmin-hminb, &
         '.a,.b max = ',hmax,hmaxb,hmax-hmaxb 
       print *,'(icedrift2:scpx)'
      call exit(1)
   endif

   call zaiord(scpy,ipmask,.false.,hmin,hmax,701)
   read(701,'(a80)') cline
   i = index(cline,'=')
   read (cline(i+1:),*) hminb,hmaxb
   if (abs(hmin-hminb).gt.abs(hminb)*1.e-4 .or. &
