...
real*8 function spherdistr8(lon1,lat1,lon2,lat2)
! --- -----------------------------------------
! --- Computes the distance between geo. pos.
! --- lon1,lat1 and lon2,lat2. 
! --- INPUT is in degrees.
! --- -----------------------------------------
   ...
   real*8  dx,dy,dz,dr                       ! Cartesian distances


   rlon1=real(lon1,kind=8)*invradian             !lon1 in rad
   rlat1=(90d0-real(lat1,kind=8))*invradian       !90-lat1 in rad 

   rlon2=real(lon2,kind=8)*invradian             !lon2 in rad
   rlat2=(90d0-real(lat2,kind=8))*invradian       !90-lat2 in rad 
   ...
   ! Use double to "elevate" product
   dr=acos(min(max(-1d0,dble(x1*x2+y1*y2+z1*z2)),1d0))     ! Acr length

   spherdistr8=dr*rearth

end function spherdistr8
