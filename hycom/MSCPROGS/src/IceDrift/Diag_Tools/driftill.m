function driftill(imem,varargin);
% illustration of ice drift from model vs obs (CERSAT)
% The icedrift routine must be run first, it creates
% the matlab files  needed here
...
elseif (nargin  >1) 
   for i=1:2:size(varargin,2)
      %varargin{i}
      %varargin{i+1}
      if (strcmp(varargin{i},'dtresh'))
         disttresh=varargin{i+1}
      end 
   end 
end
