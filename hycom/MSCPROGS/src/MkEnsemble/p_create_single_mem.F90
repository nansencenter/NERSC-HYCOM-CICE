...
#define ICE 
      ...
      character(len=8) var
      character(len=3) cmem_in
      character(len=3) cmem_out
      logical ex, lok

      real rv,rh,alp,bet, sd_d
      integer i,j,ens_in,ens_out,n1,n2,layer, level
      integer :: kdm
      integer :: ios,fab
      ...
      inquire( exist = ex , file ='create_single_mem.in') 

      if (ex) then
         open(11,file='create_single_mem.in',action='read', status='old')
         read(11,*) rstfile  !example:TP4restart2011_249_00_mem
         read(11,*) icefile
         read(11,*) ens_in  ! # ensemble members use as input
         read(11,*) ens_out ! # ensemble members output is saved
         ...
         print *,'Start file for physic and ice as well as outfile are specified'
         print *,'correlation scales and standard deviation of perturbations are specified in the input file'
         print *,'create_single_mem.in. An example of this file is found  '
         print *,'between the ----- markers below. '
         print *,'-----'
         print *,'TP4restart2011_249_00_mem'
         print *,'TP4restart2011_249_00ICE.uf'
         print *,'1            # ensemble member we are readinga as input'
         print *,'2            # perturb ensemble member saved in this mem'
         ...
         print *,'-----'
         print *
         print *,'This will genereate a perturbed file PERTTP4restart2011_249_00_mem002.[a,b] '
         print *,'from file TP4restart2011_249_00_mem001.[a,b]'
         print *,'It will also filled up the second record of the file PERTURBED_ICE.uf'
         print *,'with perturbed ice thickness reading the first record of TP4restart2011_249_00ICE.uf'
         print *,'as input '

         print *,'create_single_mem.in missing'
         call exit(1)
      end if


      filebase=trim(rstfile)
...
!      rh=25.00   ! horizontal decorrelation scale (number of grid cells)
      ...
      ! set file names
      write(cmem_in,'(i3.3)') ens_in 
      write(cmem_out,'(i3.3)') ens_out 
      infile=trim(rstfile)//cmem_in
      outfile='PERT'//trim(rstfile)//cmem_out

      print *
      print '(a)','Reading file : '//infile
      print '(a)','Dumping file  : '//outfile
...
! ICE ensemble
   if (trim(icefile) .ne. 'none') then
      ...
      call system('touch PERTURBED_ICE.uf')
      open(11,file='PERTURBED_ICE.uf',status='old',form='unformatted', &
                 access='direct',recl=j)

      rh=25. ; sd_d=0.1 ! By authority, should not be the same as ocean fields
      print *,'Creating ensemble members - ICE: rh=25. ; sd_d=0.1'
        read(10 ,rec=ens_in,iostat=ios) ficem,hicem,hsnwm,ticem,tsrfm
        call pseudo2D(ranfld,idm,jdm,1,rh,n1,n2)
        hicem=hicem*exp(ranfld*sd_d-0.5*sd_d**2)
        write(11,rec=ens_out,iostat=ios) ficem,hicem,hsnwm,ticem,tsrfm
