...
if [ $# -ne 1 ] ; then
   echo "Script to convert from hycom PHC climatology to netcdf "
...
if [ "${HYCOM_ALL}" == "" ] ; then
   echo "Specify path to hycom ALL directory in variable HYCOM_ALL"
   exit
fi
...
JTEST=0
ARCH=intel
ZINT="${HYCOM_ALL}/relax/src/z_levitus"
...
if [ "${PHC_PATH}" == "" ] ; then
   echo "Specify path to hycom climatology in environment variable PHC_PATH"
   exit
fi

if [ ! -d "${PHC_PATH}" ] ; then
   echo "${PHC_PATH} is not a directory"
   exit
fi
...
depthlevels="0.0 30.0 100.0 200.0 400.0 1000.0 2000.0 3000.0"
