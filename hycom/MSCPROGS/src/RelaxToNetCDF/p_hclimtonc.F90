...
call proj_ini()       ! Init projection


allocate(tmpx(nxp))
allocate(tmpy(nyp))
allocate(regu2d   (nxp,nyp))        ! 2D vars on regular gri
allocate(regubathy(nxp,nyp))        ! 2D vars on regular gri
...
if (trim(cprojection)=='regular') then
   call handle_err(NF90_DEF_DIM(ncid,'longitude',nxp,lon_dimension_id))
   call handle_err(NF90_DEF_DIM(ncid,'latitude',nyp,lat_dimension_id))
else if (trim(cprojection)=='ps_amsr' .or. trim(cprojection)=='polar_stereographic' .or. &
         trim(cprojection)=='native') then
   call handle_err(NF90_DEF_DIM(ncid,'x',nxp,lon_dimension_id))
   call handle_err(NF90_DEF_DIM(ncid,'y',nyp,lat_dimension_id))
else
