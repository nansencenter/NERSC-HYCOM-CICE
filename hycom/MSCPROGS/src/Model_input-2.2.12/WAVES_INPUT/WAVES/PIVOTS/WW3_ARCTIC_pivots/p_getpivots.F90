...
! --- TODO: add input argument for the wam data and grid data, read glon,glat,nlon,nlat
   ...
   implicit none
      ...
      character(len=160)  :: ncfil!! full name of input data file
                                  !! (determined using 'd_path', 'd_name' variables below)

 !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      !! Only need to change the parameters between the 2 lines of '!!!'
      !! After compilation, go to 'RUN_HERE' directory,
      !!  and run ./get_pivots.sh

      !! Description of output in 'filenameb'
      character(len=*), parameter :: out_str    = 'Pivot positions in WW3 ARCTIC related to model grid   '

      !! Names of output files;
      character(len=*), parameter :: filenamea  = 'pivots_ww3_arctic.a'!final name will be ${MODEL}_pivots_ww3.a (output renamed in RUN_HERE/get_pivots.sh)
      character(len=*), parameter :: filenameb  = 'pivots_ww3_arctic.b'!final name will be ${MODEL}_pivots_ww3.b (output renamed in RUN_HERE/get_pivots.sh)

      !! Location and name of a file with lon/lat of input data points
      !!NETDCF3:
      character(len=*), parameter :: d_path  = '/work/shared/nersc/msc/WAVES_INPUT/WW3_ARCTIC/2015/analysis_m2/'!!path of input data file
      character(len=*), parameter :: d_name  = 'SWARP_WW3_ARCTIC-12K_20151109.nc'!!local name of any input data file containing lon/lat

      ! !!NETDCF4 (not working):
      ! character(len=*), parameter :: d_path  = '/work/shared/nersc/msc/WAVES_INPUT/WW3_ARCTIC/2015/originals/2015110900/'
      ! character(len=*), parameter :: d_name  = 'SWARP_WW3_ARCTIC-12K_20151109.nc'!!local name of any input data file containing lon/lat

      !! Hardcoded grid sizes in data nc files 
      integer, parameter :: nlon=450, nlat=400
...
! --- input data dimension and lon,lat
      logical,parameter :: use_vec = .false.                !lon/lat in vec's (T) or matrices (F)
      real              :: vlat(nlat), vlon(nlon)           !vectors of lon/lat (not used if use_vec is false)
...
! --- Get lon/lat positions of input data from netcdf file:
      ...
      if (use_vec) then
         !lon/lat are given in vec's
         call ncvar_read(trim(ncfil),lon_name,vlon,nlon,1, 1,1,1)
         call ncvar_read(trim(ncfil),lat_name,vlat,nlat,1, 1,1,1)
