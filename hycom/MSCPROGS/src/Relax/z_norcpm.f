...
C
C     SETUP FOR 0.5 DEGREE FILES DERIVED FROM NORCPM.
C
      INTEGER    IWI,JWI,KWI
      REAL     XFIN,YFIN,DXIN,DYIN
      PARAMETER (IWI=720, JWI=360, KWI=35)
      PARAMETER (XFIN=0.0, YFIN=-89.75, DXIN=0.5, DYIN=0.5)
C
C     CLIM ARRAYS.
C
      INTEGER, ALLOCATABLE :: MSK(:,:)
      REAL,  ALLOCATABLE :: PLON(:,:),PLAT(:,:),XAF(:,:),YAF(:,:)
      REAL,  ALLOCATABLE :: TM(:,:),SM(:,:),RM(:,:),RT(:,:)
C
      INTEGER   KSIGMA
      REAL    XAMAX,XAMIN,YAMAX,YAMIN
      REAL    TSEAIN(IWI,JWI),SSEAIN(IWI,JWI),RSEAIN(IWI,JWI),
     +          ZLEV(KWI)
...
C
      INTEGER IBD(4)
      REAL  TSEAI(IWI+4,JWI+4),SSEAI(IWI+4,JWI+4),RSEAI(IWI+4,JWI+4)
...
C
      ...
      ! get ZLEV
      call ncheck(nf90_inq_varid(ncFIDt,'lev',ncVIDt))
      call ncheck(nf90_get_var(  ncFIDt,        ncVIDt,ZLEV(:)))
      ! inquire variable ID
      write(6,*) 'Checking for temperture ID'
...
C
     ...
     &                           SSEAIN(:,:),
     &                           (/ 1,1,KREC /) ))
C AS Remove masked values
        WHERE(SSEAIN<-1.0e4) SSEAIN=0.0
        WHERE(TSEAIN<-1.0e4) TSEAIN=0.0
C
...
C
      ...
      REAL  TSEA(IW,JW),SSEA(IW,JW),RSEA(IW,JW),RTOP(IW,JW)
C
...
C
      INTEGER IW,JW
      REAL  TSEA(IW,JW),SSEA(IW,JW),RSEA(IW,JW)
C
