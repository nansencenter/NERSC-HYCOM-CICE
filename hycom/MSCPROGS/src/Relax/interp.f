...
C
      INTEGER IW,JW,JWI
      REAL  YAF(IW,JW),PLAT(IW,JW), YAG(JWI),YFIN,WK(JWI)
C
...
C
      ...
      REAL    FLD(NDX,NY),FX(NDX,NY),FY(NDX,NY)
      REAL    FLDI(NDXI,NYI)
C
C**********
C*
...
C
      ...
      REAL    FLD( NDX, NY), FX(NDX,NY), FY(NDX,NY)
      REAL    FLDI(NDXI,NYI),FXI(NXI),FYI(NYI),
...
c
      ...
      integer m,n,npass
      integer mask(m,n)
      real    a1(m,n),a2(m,n),a3(m,n),a4(m,n),a5(m,n)
c
c --- creeping extrapolation into the land mask,
c --- using npass's of a 9-point smoother based extrapolation scheme.
c --- mask == 1 for ocean.
