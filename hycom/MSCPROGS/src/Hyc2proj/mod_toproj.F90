...
! projections they have other meanings
   ...
   ! Mercator specific parameters
   real, save :: mercator_central_lon=0.
   ...
   integer, save, dimension(:,:), allocatable, PRIVATE :: ipivs, jpivs       ! Poivot points
   real   , save, dimension(:,:), allocatable, PRIVATE :: a1s, a2s, a3s, a4s ! Bilinear coeffs
   logical,save, PRIVATE  :: testflag=.false.
...
subroutine proj_ini()
   ...
   if (trim(cprojection)=='regular') then
      ...
      do ilon=1,nxp
         ...
         call mercator(lons(ilon,jlat),lats(ilon,jlat), &
                       tmp1,tmp2,'xy2ll')
      ...
      ! Also fetch model lon/lat grid in projection coords
      do j=1,jdm
      do i=1,idm
...
subroutine get_proj()
   ...
   real :: mrc_lon_1, mrc_lat_1, mrc_lon_N, mrc_lat_N
   ...
   if (trim(cprojection)/='regular' .and. trim(cprojection)/= 'mercator' .and. &
   ...
   elseif (trim(cprojection)=='mercator') then
      read(10,*,iostat=ios) mercator_central_lon,tmpchar
      print '(a,f10.3)','mercator central longitude:', &
         mercator_central_lon

      ! Also, mercator input format is slightly different. first/last lon/lat is
      ! specified and number of points (not dx/dy) is given
      mrc_lon_1=firstx
      mrc_lat_1=firsty
      mrc_lon_N=lastx
      mrc_lat_N=lasty
      call mercator(mrc_lon_1,mrc_lat_1,firstx,firsty,'ll2xy')
      call mercator(mrc_lon_N,mrc_lat_N, lastx, lasty,'ll2xy')

      if (nint(dx)<2 .or. nint(dy)<2) then
         print *,'Mercator error - number of gris points along x and y'
         print *,'projection dimensions must be > 2 :'
         print *,'Nx=',nint(dx)
         print *,'Ny=',nint(dy)
         call exit(1)
      end if
      dx=(lastx-firstx)/real((nint(dx)-1)) ! NB - input dx is number of points in 1st proj dim
      dy=(lasty-firsty)/real((nint(dy)-1)) ! NB - input dy is number of points in 2nd proj dim
   end if
   ...
   subroutine polar_stereographic(lon,lat,x,y,direction)
      ...
      if (trim(direction)=='xy2ll') then
         ...
         if (c.eq.0.0) then 	
            lat=asin(cos(c)*sin(polar_stereographic_central_lat/CDR))*CDR
         else
   ...
   subroutine mercator(lon,lat,x,y,direction)
   implicit none
   real, intent(inout) :: x,y                ! Projection coordinate
   real, intent(inout) :: lon,lat            ! longitude / latitude
   character(len=*), intent(in) ::  &
      direction ! direction = 'xy2ll' : from projection to lon/lat
                ! direction = 'll2xy' : from lon/lat to projection 
   real, parameter :: CDR=57.29577951    ! radian to degree conv factor

   if (trim(direction)=='ll2xy') then
      x=lon-mercator_central_lon
      y=log(tan(lat/CDR)+1./cos(lat/CDR))
   else if (trim(direction)=='xy2ll') then
      lon=x+mercator_central_lon
      lat=CDR*atan(sinh(y))
   else
      print *,'Invalid direction specified'
      stop '(mercator)'
   ...
   logical function isrectilinear()
   implicit none
   isrectilinear=trim(cprojection)=='mercator' .or. trim(cprojection)=='regular'
   end function isrectilinear
