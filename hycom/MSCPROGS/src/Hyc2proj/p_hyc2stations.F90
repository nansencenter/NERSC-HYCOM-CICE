...
!         call HFReadDPField(hfile,dplayer,idm,jdm,k,1)
      ...
      ! TODO: Ahould set vector flags in fields_to_plot and put them in fld type
      do ifld=1,nfld
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!1st approach - one group per file
      ...
      do igroup = 1,ngroup
         ...
         call DefNCHeader(ncstate,hfile, cver,igroup,groupname(igroup))
...
!LB classicall putNCVar(ncstate, stations(igroup,1:nstat)%lon,nstat,1,1,'latitude',1,.false.)
         call putNCVar(ncstate, stations(igroup,1:nstat)%lat,nstat,1,1,'latitude',1,.false.)
...
#else
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!2nd approach - several groups per file
      ncfil2='test.nc'
      nstat=maxval(stations_per_group(1:ngroup))
      allocate(tmp (ngroup,nstat,ndeep))
      allocate(tmp2(ngroup,nstat,ndeep))

      call openNCFile  (ncstate,trim(ncfil2))
      call DefNCHeader(ncstate,hfile, cver)
      call DefNCDimStat(ncstate,ngroup,nstat,ndeep)
      call NCTimeVar(ncstate,hfile)
      call NCSpaceVarStat(ncstate,ngroup,nstat,deeps,ndeep) 

      ! Put lon, lat and depth. Undefine where station is undefined
      do igroup=1,ngroup
         nstat2=stations_per_group(igroup)
         stat_depth(nstat2:nstat,igroup)=undef
         stations(igroup,nstat2:nstat)%lon=undef
         stations(igroup,nstat2:nstat)%lat=undef
      end do
      call putNCVar(ncstate, stations(1:ngroup,1:nstat)%lon,ngroup,nstat,1,'longitude',2,.false.)
      call putNCVar(ncstate, stations(1:ngroup,1:nstat)%lat,ngroup,nstat,1,'latitude',2,.false.)
      call putNCVar(ncstate, transpose(stat_depth(1:nstat,1:ngroup)),ngroup,nstat,1,'model_depth',2,.false.)
      if (lev) then
         ! Unfortunate array choice means a reshape is needed
         tmp =undef
         tmp2=undef
         do igroup=1,ngroup
            nstat2=stations_per_group(igroup)
            tmp (igroup,1:nstat2,:)=stat_temp_lev(1:nstat2,1:ndeep,igroup)
            tmp2(igroup,1:nstat2,:)=stat_saln_lev(1:nstat2,1:ndeep,igroup)
         end do
         call putNCVar(ncstate,tmp ,ngroup,nstat,ndeep,'levtemp',4,.false.)
         call putNCVar(ncstate,tmp2,ngroup,nstat,ndeep,'levsaln',4,.false.)
      endif
      ...
      do ifld=1,nfld
         ...
         if (is3Dvar(hfile,fld(ifld)%fextract,1)) then
            ...
            do igroup=1,ngroup
               nstat2=stations_per_group(igroup)
               tmp (igroup,1:nstat2,:)=stat_3D_z(1:nstat2,1:ndeep,ifld,igroup)
            end do
            call putNCVar(ncstate, tmp,ngroup,nstat,ndeep,fld(ifld)%fextract,4,.false.)
         else
            tmp =undef
            do igroup=1,ngroup
               nstat2=stations_per_group(igroup)
               tmp (igroup,1:nstat2,1)=stat_2D(1:nstat2,ifld,igroup)
            end do
            call putNCVar(ncstate, tmp(:,:,1),ngroup,nstat,1,fld(ifld)%fextract,3,.false.)
         end if
      end do
     ...
     deallocate(stat_saln_lev)
     deallocate(stat_temp_lev)
