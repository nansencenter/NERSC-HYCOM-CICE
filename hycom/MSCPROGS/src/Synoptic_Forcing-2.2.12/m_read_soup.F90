...
!      use mod_year_info
      ...
      real, dimension(1-nbdy:idm+nbdy,1-nbdy:jdm+nbdy), intent(in) :: plat,plon
      real, dimension(1-nbdy:idm+nbdy,1-nbdy:jdm+nbdy), intent(inout) :: synuwind, synvwind
      real, dimension(1-nbdy:idm+nbdy,1-nbdy:jdm+nbdy), intent(inout) :: syntaux, syntauy
      real, dimension(1-nbdy:idm+nbdy,1-nbdy:jdm+nbdy), intent(inout) :: synwndspd
      real, dimension(1-nbdy:idm+nbdy,1-nbdy:jdm+nbdy) :: tmpuin, tmpvin, tmpwin
      logical :: notfound,ex

      real pi,rad,tmpw2,wfact
...
!      mlon=modlon ; where (mlon<0) mlon=mlon+360.
      mlon = plon ; where (mlon < 0.) mlon=mlon+360.

      pi=acos(-1.)
      if(mnproc==1) print *,pi
      rad=pi/180.
...
! This we don't care about at the moment...
      ...
      if (.not.ex) then
         if (mnproc==1) print *,'Cant find '//trim(path0)//rt%cyy//rt%cmm//trim(Sname)
         call flush(lp)
         call xcstop('(read_soup)')
         stop '(read_soup)'
      else
         if (mnproc==1) print *,'Reading '//trim(path0)//rt%cyy//rt%cmm//trim(Sname)
      end if
      open(20,file=trim(path0)//rt%cyy//rt%cmm//trim(Sname),form='formatted')
      Read(20,'(a80)') header
      if (mnproc==1) print *,'Header : ',header
      ind=index(header,'Format')
      read(header(ind+6:),*) ifirst,ilast
      if (mnproc==1) print *,header
      if (mnproc==1) print *,ifirst,ilast
      ...
      if (mnproc==1) print *,'first=',ifirst,lwyy,lwmonth,lwdd,lwhh
      ...
      if(mnproc==1) print *,'last=',ilast,hiyy,himonth,hidd,hihh
      ...
      if(mnproc==1) print *,'baserec,low,hi:',baserec,lwrec,hirec
      ...
      call bilin_soup(windx  ,nlon,nlat,flon,flat,dlon,dlat,tmpu,idm,jdm,mlon,plat)
      call bilin_soup(windy  ,nlon,nlat,flon,flat,dlon,dlat,tmpv,idm,jdm,mlon,plat)
      call bilin_soup(weights,nlon,nlat,flon,flat,dlon,dlat,tmpw,idm,jdm,mlon,plat)
...
!
     ...
     do j=1,jdm
         ...
         do i=1,idm
            ...
            tmpw2=MAX(2.5,MIN(32.5,synwndspd(i,j)))
            cd_new = 1.0E-3*(.692 + .0710*tmpw2 - .000700*tmpw2**2)
