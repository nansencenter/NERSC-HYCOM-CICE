...
contains
   ...
   subroutine read_ncep(rt,plon,plat,depths)
      ...
      !fhgt =trim(ncep_path)//'hgt.sfc.nc' ! Old 
      fhgt =trim(ncep_path)//'hgt.sfc.gauss.nc'
...
!NCEP relative humidity and pressure
      ...
      if (lsynslp .or. lsynrelhum) then
         ...
         synslp(:,:)=synslp+hgt*9.8*1.3*0.01 ! Correction for geopot. height
