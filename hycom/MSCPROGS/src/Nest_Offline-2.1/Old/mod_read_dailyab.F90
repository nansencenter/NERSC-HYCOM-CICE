...
contains
 ...
 118  format ('member ',i5.5,' = ',i1,' Ensemble member flag')
   ...
   if (yrflag==3) then
      call year_day(real(rtdump%idd,8),rtdump%iyy,rtdump,'ecmwf')
      call year_day(real(rtinit%idd,8),rtinit%iyy,rtinit,'ecmwf')
   elseif (yrflag==0) then
      call year_day(real(rtdump%idd,8),rtdump%iyy,rtdump,'month')
      call year_day(real(rtinit%idd,8),rtinit%iyy,rtinit,'month')
   else
