...
! --- -----------------------------------------
   ...
   ! KAL -- The routines below lead to lack of precision
   !     -- so we hardcode reals to real*8 in temp calcs
   ...
   !print '(9f14.8,e14.5)',z1-z2,x1-x2,y1-y2,x1,x2,y1,y2,z1,z2,&
   !x1*x2+y1*y2+z1*z2
   dott=min(x1*x2+y1*y2+z1*z2,1.)
   !dr=DSQRT(dx*dx+dy*dy+dz*dz)       !distance pytagaros
   !dr=Dacos(x1*x2+y1*y2+z1*z2)       ! Arc length
   dr=Dacos(dott)       ! Arc length
