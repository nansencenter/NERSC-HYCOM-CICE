...
contains
   ...
   subroutine read_gpheader(rungen,dirname)
   implicit none
   ...
   if (.not. present(dirname)) then
      ...
      gphdr=trim(dirname)//'/'//rungen//'gprecord.asc'
   end if
   inquire(exist=ex,file=gphdr)
   if (.not.ex) then
      print *,'No gp header file '//trim(gphdr)
      stop '(read_gpheader)'
   end if
   inquire(exist=ex,file=gphdr)
   open(nop,file=trim(gphdr), status='old')
   read(nop,*) ! Skip first line
   ios=0
   nvars=0
   do while (ios==0)
      read(nop,'(a12,a3)',iostat=ios) c12dummy,i
   ...
   kdm=-100
   do ivar=1,nvars
      read(nop,'(a12,i3)') varname(ivar),vardim(ivar)
