...
#ifdef _FABM_
          elseif (trcflg(ktr).eq.906) then
! ---       FABM
            call hycom_fabm_initialize()
#endif
          endif
        enddo
        close(unit=uoff+99)
...
!
#ifdef _FABM_
!     Analytic [spatially homeogeneous] initialization, even when using
!     climatology, because climatological fields may be available for a
!     subset of tracers only.
      call hycom_fabm_initialize_state()
#endif
...
!    &                    trcij,prij,kk        )
              ...
              do k=1,kk
                do ktr= 1,ntracr
#ifdef _FABM_
                  if (hycom_fabm_relax(ktr) == -1) cycle
#endif
...
! ---     NPZD
        ...
        elseif (trcflg(ktr).eq.906) then
! ---     FABM
          call hycom_fabm_update(m, n, ktr)
!       elseif (trcflg(ktr).eq.909) then
! ---     Chai 9-component.
