...
!
!KAL
!KAL  function restart_name(dtimex, yrflag, dir)
...
!
!
!     If dir is "in" and name ends with restart_in, dont touch it.
      if (trim(dir) == "in") then
         restart_name=trim(flnmrsi)
         n=len_trim(restart_name)
!ALFA         if (n>=10 .and. restart_name(n-9:n) == "restart_in") then
        if (n>=10 ) then
           if (restart_name(n-9:n) == "restart_in") then
             return
           end if
         end if
!     If dir is "out" and name ends with restart_out, dont touch it.
...
!
!     If still here ... Append dateinfo after last dot in flnmrs
...
!
!     Time format is always iyear_jday_hour_sec (ex 2001_005_12_0000)
