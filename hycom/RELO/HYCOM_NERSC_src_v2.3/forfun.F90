...
!     CAGLAR: BEGIN (MAY2019)
      ...
      if     (mnproc.eq.1) then  ! .b file from 1st tile only
      open (unit=uoff+926,file=flnmfor(1:lgth)//'forcing.dewpt.b', &
         status='old', action='read')
      read (uoff+926,'(a79)') preambl
      endif !1st tile
      call preambl_print(preambl)
      call rdmonth(util1, 926)
!     CAGLAR: END
#endif
      if     (sstflg.eq.3) then
...
!
      ...
      i = index(cline,'=')
!KAL  if     (iunit.ge.900 .and. iunit.le.910) then
...
! ---   shortwave through sea ice is handled separately
        ...
        do j= 1-nbdy,jj+nbdy
          do i= 1-nbdy,ii+nbdy
             swflxdwn(i,j,1) = swflxdwn(i,j,2) ! Done in rdpall1 for swflx
             swflxdwn(i,j,2) = swflx(i,j,2)
          enddo
        enddo
#endif
        if     (albflg.eq.1) then
...
! ---     shortwave through sea ice is handled separately
          ...
          do j= 1-nbdy,jj+nbdy
            do i= 1-nbdy,ii+nbdy
              swflxdwn(i,j,lslot) = swflx(i,j,lslot)
            enddo
          enddo
#endif
          if     (albflg.eq.1) then
...
!
        if (relaxt) then
#ifdef _FABM_
          call hycom_fabm_relax_read(lslot, mnth)
#else
          do ktr= 1,ntracr
            do k= 1,kk
...
!
        ...
        enddo
#ifdef _FABM_
        call zaiopf('nest/rmutr.a', 'old', 915)
        if     (mnproc.eq.1) then  ! .b file from 1st tile only
        open (unit=uoff+915,file='nest/rmutr.b', &
              status='old',action='read')
        endif !1st tile
        call zagetc(cline,ios, uoff+915)  !1st line of the header on all tiles
        if     (mnproc.eq.1) then  ! .b file from 1st tile only
        ...
        call preambl_print(preambl)
        call rdmonth(rmunp_trc, 915)
        call xctilr(rmunp_trc,1,1, nbdy,nbdy, halo_ps)
        if     (mnproc.eq.1) then  ! .b file from 1st tile only
        close (unit=uoff+915)
        endif !1st tile
        call zaiocl(915)
...
!
#ifdef _FABM_
      call hycom_fabm_nest_read(iyear,iday,ihour,lslot)
#endif
      return
      end
!
