...
#ifdef _FABM_
      call hycom_fabm_configure()
      ntracr = size(fabm_model%interior_state_variables)
#else
      ntracr = abs(ntracr)
#endif
      trcout = ntracr.ne.0
...
!
      ...
      relaxs = .not. relax &
               .and. relaxf
#if defined(NERSC_HYCOM_CICE)
!KAL  Read filenames. NB: This can get overwritten below 
      call blkins(flnmrsi ,'nmrsti','restart_in'       ,.true.)
      call blkins(flnmrso ,'nmrsto','restart_out'      ,.true.)
      call blkins(flnmarcs,'nmarcs','archs.0000_000_00',.true.)
      call blkins(flnmarc ,'nmarcv','archv.0000_000_00',.true.)
      call blkins(flnmarcm,'nmarcm','archm.0000_000_00',.true.)
!KAL
#endif
!      
#if defined(STOKES)
...
!KAL  For reading strings
      ...
      implicit none
!
      character(len=*),intent(out):: svar
      character(len=6),intent(in) :: cvar
      character(len=*),intent(in) :: sdefault
      logical,         intent(in) :: append_dot
      integer     ivar,itmp,ldot
      character*256 tmp
      character*6 cvarin
!
! --- read in one string
      read(uoff+99,'(a)',iostat=itmp) tmp
      if (itmp /= 0) then
         tmp=""
      endif
!
! --- NB: ivar is ignored for now, but can be used to indicate type of datestring, etc
      itmp = index(tmp,"=")
      svar=""
      cvarin=""
      if (itmp > 0 ) then
         read(tmp(1:itmp),*) ivar,cvarin
         read(tmp(itmp+1:len_trim(tmp)),*) svar
      endif
!     if (mnproc.eq.1) then
!        print *,itmp
!        print *,tmp(1:itmp)
!        print *,tmp(itmp+1:len_trim(tmp))
!     end if
!
! --- Check that we read the correct variable
      if     (cvar.ne.cvarin) then 
        if (mnproc.eq.1) then
        write(lp,*) 
        write(lp,*) 'error in blkins - input ',cvarin, &
                             ' but should be ',cvar
        write(lp,*) 
        call flush(lp)
        endif !1st tile
...
! --- Empty string, return default
      if (len_trim(svar) == 0) then
         svar=sdefault
         if (mnproc==1) then 
            write(lp,'(a,a,a)')  &
            "NB: Setting ",trim(cvar)," to default value "
            write(lp,6000) cvarin,ivar,trim(svar)
         end if
         return
      end if
!
! --- Append dot if requested and needed..
      ldot=index(svar,".",back=.true.)
      if (ldot==0 .and. append_dot) then
         if (mnproc.eq.1)  &
            write(lp,'(a,a)') "NB: Appending . to file name ",trim(svar)
         svar=trim(svar)//"."
      endif
!
      if (mnproc.eq.1) then
