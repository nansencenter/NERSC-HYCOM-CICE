...
!
!KAL  Moved to state
!KAL  logical, save, public  :: put_export   !set in main program
!KAL  logical, save, public  :: get_import   !set in main program
!KAL  logical, save, public  :: end_of_run   !set in HYCOM_Run
...
!
! --- Export Fields
      integer, parameter :: numExpFields=23 !KAL
...
! read atm for both HYCOM and CICE in HYCOM
      ...
      expFieldLongName( 9) = ""
      expFieldStdName(  9) = ""
      expFieldUnits(    9) = "m s-1"
      expFieldName(    10) = "vair"
      expFieldLongName(10) = ""
      expFieldStdName( 10) = ""
      expFieldUnits(   10) = "m s-1"
      expFieldName(    11) = "zair"
      expFieldLongName(11) = ""
      expFieldStdName( 11) = ""
      expFieldUnits(   11) = "m"
      expFieldName(    12) = "pott"
      expFieldLongName(12) = ""
      expFieldStdName( 12) = ""
      expFieldUnits(   12) = "K"
      expFieldName(    13) = "ss_tltx"
      expFieldLongName(13) = "Sea Surface slope x"
      expFieldStdName( 13) = ""
      expFieldUnits(   13) = "m m*-1"
      expFieldName(    14) = "ss_tlty"
      expFieldLongName(14) = "Sea Surface Slope y"
      expFieldStdName( 14) = ""
      expFieldUnits(   14) = "m m*-1"
      expFieldName(    15) = "Qair"
      expFieldLongName(15) = "Specific humidity of air"
      expFieldStdName( 15) = ""
      expFieldUnits(   15) = "kg kg**-1"
      expFieldName(    16) = "rhoair"
      expFieldLongName(16) = "Density of air"
      expFieldStdName( 16) = ""
      expFieldUnits(   16) = "kg m**-3"
      expFieldName(    17) = "swvdr"
      expFieldLongName(17) = "sw down, visible, direct"
      expFieldStdName( 17) = ""
      expFieldUnits(   17) = "W m**-3"
      expFieldName(    18) = "swvdf"
      expFieldLongName(18) = "sw down, visible, diffuse"
      expFieldStdName( 18) = ""
      expFieldUnits(   18) = "W m**-3"
      expFieldName(    19) = "swidr"
      expFieldLongName(19) = "sw down, infrared, direct"
      expFieldStdName( 19) = ""
      expFieldUnits(   19) = "W m**-3"
      expFieldName(    20) = "swidf"
      expFieldLongName(20) = "sw down, infrared, diffuse"
      expFieldStdName( 20) = ""
      expFieldUnits(   20) = "W m**-2"
      expFieldName(    21) = "flw"
      expFieldLongName(21) = "Longwave radiation, downward"
      expFieldStdName( 21) = ""
      expFieldUnits(   21) = "W m**-2"
      expFieldName(    22) = "rrate"
      expFieldLongName(22) = "Rainfall Rate"
      expFieldStdName( 22) = ""
      expFieldUnits(   22) = "kg m**-2 s*-1"
      expFieldName(    23) = "srate"
...
! 
              t2f1  = (spcifh*dp1)/(baclin*icefrq*g)
              t2f2  = (spcifh*dp2)/(baclin*icefrq*g)
! 
              tfrz1 = CICE_tf(saln(i,j,k,n),tfrz_opt)
              tfrz2 = CICE_tf(saln(i,j,k,m),tfrz_opt)
! 
! ----------- Integrated freezing potential
              ssfi =  ssfi + (tfrz1-temp(i,j,k,n))*t2f1  &
                           + (tfrz2-temp(i,j,k,m))*t2f2
! 
! ----------- Averaged salinity and temperature
              smxl = smxl + dp1*saln(i,j,k,n) + dp2*saln(i,j,k,m)
              tmxl = tmxl + dp1*temp(i,j,k,n) + dp2*temp(i,j,k,m)
              psur1 = psur1 + dp1
              psur2 = psur2 + dp2
              if     (min(psur1,psur2).ge.hfrz) then
                exit
              endif
            end do
...
!KAL
! ---       Read Air temp and vapor mixing ratio
            if     (flxflg.ne.3) then
              if     (natm.eq.2) then
               ...
               call xchalt('Export_ESMF: unhandled flxflg=3')
               stop 'Export_ESMF: unhandled flxflg=3'
            endif
            airt=airt + tzero               ! in Kelvin
            expData(8)%p(i+i0,j+j0) = airt
!
! ---       Read wind speed. Wind in P-cell center
            if (wndflg.eq.2.and. .not. stroff) then
               if     (natm.eq.2) then
                 w_s = wndspd(i,j,l0)*w0+wndspd(i,j,l1)*w1
               else
                 w_s = wndspd(i,j,l0)*w0+wndspd(i,j,l1)*w1 &
               ...
               t_x = surtx(i,j)
               t_y = surty(i,j)
               t_dir = atan2(t_y,t_x) ! Angle on computational grid
               wndx= w_s*cos(t_dir)    ! x component of wind
               wndy= w_s*sin(t_dir)    ! y component of wind
            elseif (wndflg.ge.4) then
               ...
               call xchalt('Setup_ESMF: unhandled windflag')
               stop 'Setup_ESMF: unhandled windflag'
            end if
            expData( 9)%p(i+i0,j+j0) = wndx  ! uair
            expData(10)%p(i+i0,j+j0) = wndy  ! vair
!
! --------- SLP if present
...
!
            expData(11)%p(i+i0,j+j0) = 10.0 ! zair
            expData(12)%p(i+i0,j+j0) = airt ! Actually pot air temp, but should be the same at zlvl
!
! --------- Sea surface slope in x- and y- direction. Centered diff in
! --------- p-cell center
            ss_tltx = (srfhgt(i+1,j)-srfhgt(i-1,j))*0.5*ssh2m/scpx(i,j)
            ss_tlty = (srfhgt(i,j+1)-srfhgt(i,j-1))*0.5*ssh2m/scpy(i,j)
            expData(13)%p(i+i0,j+j0) = ss_tltx
            expData(14)%p(i+i0,j+j0) = ss_tlty
!
! --------- vapour mixing ration ~ specific humidity
            expData(15)%p(i+i0,j+j0) = vpmx
!
! --------- Density of air at sea level pressure
            !expData(16)%p(i+i0,j+j0) = pair / ( rgas * tair )
            expData(16)%p(i+i0,j+j0) = pair /  &
               ( rgas * airt * (1.0+0.608*vpmx) )
!
! --------- radflx is downwelling longwave when lwflag=-1
            if (lwflag.eq.-1) then
               if     (natm.eq.2) then
               ...
               call xchalt('Setup_ESMF: unhandled lwflag<>-1')
               stop 'Setup_ESMF: unhandled lwflag<>-1'
            end if
!
! --------- SW down, visible, direct. diffuse and infrared components set to 0.
! --------- NB: HYCOM swflx input files are downwelling shortwave if albflg>0, but it
! --------- is modified in forfun so that swflx is net shortwave at
! --------- surface. swflxdwn is a array added to keep unmodified downwelling SW
! --------- and pass it to CICE. 
            if (albflg.gt.0) then
               if     (natm.eq.2) then
                 qswdwn=(swflxdwn(i,j,l0)*w0+swflxdwn(i,j,l1)*w1)
               else
                 qswdwn=(swflxdwn(i,j,l0)*w0+swflxdwn(i,j,l1)*w1 &
                        +swflxdwn(i,j,l2)*w2+swflxdwn(i,j,l3)*w3)
               endif !natm
            else 
               call xchalt('Setup_ESMF: unhandled albflg<=0')
               stop 'Setup_ESMF: unhandled albflg<=0'
            end if
            expData(21)%p(i+i0,j+j0) = radfl
            if (albflg.gt.0) then
! ----------  divided the downward shortwave into four components
! ----------  reference to ice_forcing.F90 -----       
...
!
! --------- We dont handle cases where input is E-P. 
            if (empflg.ne.3) then
               flagtemp = 0.5*(1.+sign(1.,airt-tzero)) ! 1 if airt > tzero, otherwise 0
               expData(22)%p(i+i0,j+j0) =    flagtemp  * prcp * 1e3 ! Rain , conversion m/s -> kg m**-2 s**-1
               expData(23)%p(i+i0,j+j0) =(1.-flagtemp) * prcp * 1e3 ! Snow , conversion m/s -> kg m**-2 s**-1
            else 
...
! --- NB: May need to set vland first...
      ...
      call xctilr(sitx_import,1,1,nbdy,nbdy,halo_pv)  !Sea Ice X-Stress
      call xctilr(sity_import,1,1,nbdy,nbdy,halo_pv)  !Sea Ice Y-Stress
      call xctilr(siqs_import,1,1,nbdy,nbdy,halo_ps)  !Solar Heat Flux thru Ice to Ocean
      call xctilr(sifh_import,1,1,nbdy,nbdy,halo_ps)  !Ice Freezing/Melting Heat Flux
      call xctilr(sifs_import,1,1,nbdy,nbdy,halo_ps)  !Ice Freezing/Melting Salt Flux
      call xctilr(sifw_import,1,1,nbdy,nbdy,halo_ps)  !Ice Net Water Flux
      call xctilr( sit_import,1,1,nbdy,nbdy,halo_ps)  !Sea Ice Temperature
      call xctilr( sih_import,1,1,nbdy,nbdy,halo_ps)  !Sea Ice Thickness
      call xctilr( siu_import,1,1,nbdy,nbdy,halo_pv)  !Sea Ice X-Velocity
      call xctilr( siv_import,1,1,nbdy,nbdy,halo_pv)  !Sea Ice Y-Velocity
      if     (iceflg.ge.2 .and. icmflg.ne.3) then
        call xctilr(covice,1,1,nbdy,nbdy,halo_ps)  !Sea Ice Concentration
        call xctilr(  si_c,1,1,nbdy,nbdy,halo_ps)  !Sea Ice Concentration
        call xctilr( si_tx,1,1,nbdy,nbdy,halo_pv)  !Sea Ice X-Stress into ocean
        call xctilr( si_ty,1,1,nbdy,nbdy,halo_pv)  !Sea Ice Y-Stress into ocean
        ...
        call xctilr(wflice,1,1,nbdy,nbdy,halo_ps)  !Sea Ice Water Flux
        ...
        call xctilr(  si_h,1,1,nbdy,nbdy,halo_ps)  !Sea Ice Thickness
...
! --- 
      if (trim(CICE_tfrz_option)=="linear_salt") then
         tfrz_opt = 1
      elseif (trim(CICE_tfrz_option)=="mushy_layer" .or.  &
              trim(CICE_tfrz_option)=="mushy") then
         tfrz_opt = 2
...
#else
      if (ESMF_LogMsgFoundError(rc, &
         "hycom_run: attributeget get_import", rcToReturn=rc2)) &
         call ESMF_Finalize(rc=rc)
#endif
!KAL
...
#else
      if (ESMF_LogMsgFoundError(rc, &
         "hycom_run: attributeget put_export", rcToReturn=rc2)) &
         call ESMF_Finalize(rc=rc)
#endif
!KAL
      if     (put_export) then
...
!KAL    Falls back to standard HYCOM behaviour
        if (trim(flnmra)=="restart_out") then
           flnmra = flnmrso  !.a extension added by restart_out
           flnmrb = flnmrso  !.b extension added by restart_out
!KAL    New behaviour - appends date info to restart file
        else 
           flnmrb = trim(flnmra)//".b"
           flnmra = trim(flnmra)//".a"
        end if
...
# if ! defined (ESPC_COUPLE)
      ...
      type(ESMF_TimeInterval)    :: timeStep, runDuration, tmpDt
      type(ESMF_Time)            :: startTime, endTime, refTime
      character(ESMF_MAXSTR)     :: msg, gridName

      integer :: rc, rc2, my_sec
