...
C
      DO J= 1,JDM
        DO I= 1,IDM
C          IF     (ABS(DEPTH(I,J)).LT.2.0**90) THEN
          IF     (ABS(DEPTH(I,J)).LT.1e20) THEN
...
c ---   output is in "*.[AB]"
     ...
     &               IVERSN,IEXPT,YRFLAG,IDM,JDM
      ...
      ENDIF
C HERE WE SET THE VALUE OF TIME FROM MODEL DAY IN INPUT [b] FILE 
      TIME=MODEL_DAY
C
...
C 
      ...
      DO J= 1,JDM
        DO I= 1,IDM
C          IF     (DEPTH(I,J).GT.ZERO) THEN
           IF(DEPTH(I,J).GT.0.0.and.
     &                      DEPTH(I,J).LT.0.5*SPVAL) THEN
...
C            CALL FIND_DENSITY(RZLOC,SIGMAA,RZ(1,I,J),KZ+1,1)
            IF     (RZLOC.EQ.0.0) THEN
C              PMIX(I,J) = ZL(2)
            ELSEIF (RZLOC.EQ.KZ+1) THEN
C              PMIX(I,J) = DEPTH(I,J)
            ELSE
C              L = RZLOC
C              Q = RZLOC - L
C              PMIX(I,J) = (1.0-Q)*ZL(L) + Q*ZL(L+1)
            ENDIF
C            PMIX(I,J) = MAX( THKMIN, MIN( PMIX(I,J), DEPTH(I,J) ) )
C              if (ldebug.and.i.eq.itest .and. j.eq.jtest) then
C                WRITE(6,'(A,2F10.3)')
C     +            'PMIX,RZLOC =',PMIX(I,J),RZLOC
C                call zhflsh(6)
C              endif !debug
          ELSE
            PCM0(I,J) = ZERO
...
C            IF     ( (ABS(DEPTH(I,J)).LT.2.0**99) .AND.
C     &   (MAX(TZ(K,I,J),SZ(K,I,J) ).LT.2.0**99) )THEN
            IF     ((DEPTH(I,J)).GT.ZERO) THEN
        ...
        if     (flag_th.ne."NONE") then
            ...
            DO J= 1,JDM
                DO I= 1,IDM
                    ...
                    if ( I .eq. 448 .and. J .eq. 425) then
                       print *,'in the body ',PM(I,J),PKM1(I,J)
                    endif
                ENDDO
            ENDDO
            CALL ZAIOWR(WORK,MSK,.TRUE.,  XMIN,XMAX, 21, .FALSE.)
...
C
 ...
 4200 FORMAT(
     + 'HYCOM archive from MERCATOR-PHY-24 for month ',I2.2,'.' /
     + A80/A80/
...
C
     ...
     +                            SIGMAA,  RZ,KZ+1,KZTOP)
                ...
                if (i.eq.448 .and. j.eq.425) then
                    print *,'K.LT.KDM  ',PZBOT,PKM1,PZTOP,SIGMAA
                    print *,rz(1:3)
                endif
...
C MOSTAFA
                ...
                ELSE
                  ...
                  Q = PZBOT - L
                  if (i.eq.448 .and. j.eq.425) then
                    print *,'MY INPS:  L=',L,'  Q= ',Q,' PZBOT=',PZBOT
                    print *,'MY INPS:  PKM1=',PKM1,' Z= ',ZL(L)
...
C
     ...
     &     K,coord,1,tlevel1,nrec,trim(archvfile)//".a")
        ...
        TZ1(448,425)=.5*(TZ1(447,421)+TZ1(449,425))
        SZ1(448,425)=.5*(SZ1(447,421)+SZ1(449,425))
        TZ1(448,424)=TZ1(448,425)
        TZ1(448,423)=TZ1(448,425)
        TZ1(448,426)=TZ1(448,425)        
        SZ1(448,424)=SZ1(448,425)
        SZ1(448,423)=SZ1(448,425)
        SZ1(448,426)=SZ1(448,425)
        TZ1(447,425)=.5*(TZ1(446,421)+TZ1(449,425))
        SZ1(447,425)=.5*(SZ1(446,421)+SZ1(449,425))
        TZ1(447,424)=TZ1(447,425)
        TZ1(447,423)=TZ1(447,425)
        TZ1(447,426)=TZ1(447,425)
        SZ1(447,424)=SZ1(447,425)
        SZ1(447,423)=SZ1(447,425)
        SZ1(447,426)=SZ1(447,425)
...
C ---    U AND V ARE NOW ON P-CELL
         ...
         DO J= 1,JDM
            DO I= 1,IDM               ! assign 3D
                 if     (flag_t.ne."NONE") then
                     TZ1(I,J) = MIN(SPVAL_,TZ1(I,J)) ! layerthickness
                     call GAP_FILL(IDM,JDM,TZ1,I,J,1000.,TZ1(I,J))
                 endif
                 if     (flag_th.ne."NONE")  
     &                  TH(K,I,J) = MIN(SPVAL_,TH1(I,J)) ! layer thickness
                 if     (flag_s.ne."NONE") then
                        SZ1(I,J) = MIN(SPVAL_,SZ1(I,J)) ! salinity
                        call GAP_FILL(IDM,JDM,SZ1,I,J,1000.,SZ1(I,J))
                        SZ(K,I,J)=SZ1(I,J)
                 endif
                 if     (flag_u.ne."NONE")
     ...
     &                      DEPTH(I,J).LT.0.5*SPVAL) THEN
                    ...
                    if (i.eq. 458 .and. j.eq. 425 ) THEN
                       print *,TZ(K,I,J),SZ(K,I,J),RZ(K,I,J)
                    ENDIF
                  ENDIF ! DEPTH-IF
               END DO
            END DO
...
C
        ...
        SUBROUTINE GAP_FILL(IDM,JDM,X,I,J,TOL,Y)
        IMPLICIT NONE
        INTEGER, INTENT(IN)  :: IDM,JDM
        INTEGER,  INTENT(In) :: I,J
        REAL*4,  INTENT(IN)  :: X(IDM,JDM),TOL
        REAL*4,  INTENT(OUT)  :: Y
        
        REAL*4    :: sgn1,sgn2,sgn3,sgn4
        ...
        sgn4=.25
        Y=X(I,J)
        if ((I.gt.50 .and. I.lt.IDM-50).and.
     &       (J .gt.50 .and. J .lt. JDM-50)
     &       .and. ABS(X(I,J)) .gt. TOL) then
           if (ABS(X(I-1,J)) .gt. TOL) sgn1=0.
           if (ABS(X(I+1,J)) .gt. TOL) sgn2=0.
           if (ABS(X(I,J-1)) .gt. TOL) sgn3=0.
           if (ABS(X(I,J+1)) .gt. TOL) sgn4=0.
           Y=(sgn1*X(I-1,J)+sgn2*X(I+1,J)
     &                      +sgn3*X(I,J-1)+sgn4*X(I,J+1) )
        endif

     
        END SUBROUTINE GAP_FILL
