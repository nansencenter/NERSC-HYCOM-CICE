...
c
     ...
     &       ,plotbl,plotml
     &       ,smooth,mthin,mdens,initl,lhycom,icegln,lsecanom
...
c --- montqq and baroqq are optional (must have montqq for baroqq)
c --- ssshqq and nsshqq are optional (must have ssshqq for nsshqq)
c --- can't have both montqq and ssshqq.
      call blkinr3(qqin,i,
     &             'montqq','("blkinr: ",a6," =",f11.4," cm")',
     &             'ssshqq','("blkinr: ",a6," =",f11.4," cm")',
     &             'sshqq ','("blkinr: ",a6," =",f11.4," cm")')
      if     (i.eq.1) then !'montqq'
        if (qqin.ge.0.0) then
        if (kpalet.ge.2 .and. qqin.eq.0.0) then
...
c ---   'center' = central contoured value
     ...
     &            0,label(33:81),0.0,nquad,.true.,lalolb,lalogr)
      ...
      call region_label(region, xlab0,ylab0,csb,
     &                  qq,     xlab1,ylab1,ctrc_format(ktr),
     &                                  trim(ctrc_units(ktr)),
     &                  amn,amx,xlab2,ylab2,ctrc_format(ktr),csn)
      if (nquad.eq.0) call fram(ncount)
      endif
      enddo !ktr
c
