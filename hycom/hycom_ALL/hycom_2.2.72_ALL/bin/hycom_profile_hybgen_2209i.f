...
c --- 'hybmap' = hybrid   remapper  flag (0=PCM,1=PLM,2=PPM,-ve:isoPCM)
c ---             (use hybmap=-3 to recover pre-2.2.09 behaviour)
...
c
      qhybrlx = 1.0/hybrlx
      isopcm  = hybmap.lt.0  !use PCM for isopycnal layers?
      hybmap  = abs(hybmap)
C
      CLOSE(99)
C
...
C
     ...
     +            DP00I,DP0K,DP0KP,DS0K,DSSK,DEPTHS,ISOTOP,THKBOT,
     +            LDEBUG)
C
      DO K= 1,KDM
        P(K+1) = P(K) + DP(K)/ONEM
