...
c
      ...
      integer i,j,kk,n, nstep, i0,j0,itest,jtest
      integer klist(1,1)
...
c ---   do nothing.
     ...
     &                          dp0ij(k-2))
        if (p_hat2.lt.p(i,j,k-1)-onemm) then
          p(i,j,k-1)=(1.0-qhybrlx)*p(i,j,k-1) + 
     &                    qhybrlx *max(p_hat2,2.0*p(i,j,k-1)-p_hat)
          if (i.eq.itest .and. j.eq.jtest) then
...
c ---     do nothing.
     ...
     &                            dp0ij(k-3))
          if (p_hat3.lt.p(i,j,k-2)-onemm) then
            p(i,j,k-2)=(1.0-qhybrlx)*p(i,j,k-2) +
     &                    qhybrlx *max(p_hat3,2.0*p(i,j,k-2)-p(i,j,k-1))
            if (i.eq.itest .and. j.eq.jtest) then
...
c
        q = p_hat - p(i,j,k)
        p(i,j,k)=max(pres(k-1),min(pres(k+1),p(i,j,k)+qhybrlx*q))
c ---   qhybrlx is relaxation coefficient (inverse baroclinic time steps)
        if (i.eq.itest .and. j.eq.jtest) then
...
c
        p_hat0 = p_hat
        q      = p_hat - p(i,j,k)
        p_hat  = max(pres(k-1),min(pres(k+1),p(i,j,k)+qhybrlx*q))
c ---   qhybrlx is relaxation coefficient (inverse baroclinic time steps)
...
c ---   entrain layer k+1 water into layer k. 
        p(i,j,k+1)=(1.0-qhybrlx)*p(i,j,k+1) + qhybrlx*p_hat
        if (i.eq.itest .and. j.eq.jtest) then
