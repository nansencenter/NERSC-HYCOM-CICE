...
c
      logical ldouble,lshift,ldiscard,luniform,l1uniform,lskip
...
c
      ...
      enddo
c
c --- handle all region types except arctic (use partit_arctic for nreg=3)
c --- nreg = closed:0; periodic:2; arctic:3; f-plane:4
c
      if     (sum(ip(1:idm-1,jdm)).ne.0) then
c
c ---   f-plane, periodic in 2nd dimension
c
        if     (.not. (luniform .or. l1uniform)) then
          write(6,'(/ a /)')
     &      'error - f-plane grid must use equal-size tiles'
          call zhflsh(6)
          stop
        endif
        do i= 1,idm
...
c
        ip(    0,:) = ip(idm,:)
        ip(idm+1,:) = ip(  1,:)
        nreg = max( nreg, 2 )  ! periodic or f-plane
      else
        ip(    0,:) = 0
        ip(idm+1,:) = 0
        if     (nreg.eq.3) then
...
c --- map halo.
c
      if     (nreg.lt.4) then  ! not f-plane
        map(1:idm,1-mbdy:0)        = 0
        map(1:idm, jdm+1:jdm+mbdy) = 0
      else                     ! f-plane
        do j= 1,mbdy
          do i= 1,idm
            map(i,  1-j) = map(i,jdm+1-j)
            map(i,jdm+j) = map(i,      j)
          enddo
        enddo
      endif
...
c ---   optionally discard, shift and/or shrinkwrap tiles over land.
c
...
*           jjpe(mpe  -j/2) = jdm/mpe + 1  ! edge   tiles larger
        ...
        enddo
c
c ---   optinally discard all-land tiles.
c
        nmpe = 0
        do j= 1,mpe
          lskip = nreg.eq.4 .and. (j.eq.1 .or. j.eq.mpe)
          do i= 1,npe
            i_w = ispt(i,j)
...
c ---       assume N-S exchange is before E-W exchange.
     ...
     &             sum(map(i_e-mbdy+1:i_e,j_n+1:j_n+mbdy))
            if     (.not. lskip .and. ldiscard .and. nsea.eq.0) then
              ispx(i,j) = 0
              iipx(i,j) = 0
            else
              nmpe = nmpe + 1
...
c
        if (lshift) then
        do j= 1,mpe
          if     (nreg.eq.4 .and. (j.eq.1 .or. j.eq.mpe)) then
            cycle ! skip first and last row when they are periodic
          endif
          j_s = jspt(  j) - mbdy
