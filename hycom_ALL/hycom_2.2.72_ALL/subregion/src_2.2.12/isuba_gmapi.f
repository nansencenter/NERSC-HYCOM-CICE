...
c
      ...
      integer              :: idm_out,jdm_out,maxinc
      integer              :: i,ii,ios,im,inc,incmax,ip,iq,ix,itmp,
     &                        j,jj,jj1,jm,jp,jq,l,
     &                        ni,nir,no,nor
      integer              :: k,l0,l1
...
c
      call xcspmd
      call zaiost
      call blkdat(idm_out,jdm_out, maxinc,
     &            chead, flnm_reg,flnm_map)
...
c
     ...
     &             maxval(abs(plat_out(1,:)-plat_out(i,:))).le.1.e-2
        if     (.not.laxis(2)) then
*         write(6,'(a,i5)') 'laxis(2)==F for i=',i
*         do j= 1,jdm_out
*           if     (abs(plat_out(1,j)-plat_out(i,j)).gt.1.e-2) then
*             write(6,'(a,2i5,2f10.4,1pe14.6)')
*    &          'i,j,lat,dif = ',i,j,plat_out(1,j),plat_out(i,j),
*    &                               plat_out(1,j)-plat_out(i,j)
*           endif
*         enddo !j
          exit
        endif
     ...
     &             maxval(abs(plon_out(:,1)-plon_out(:,j))).le.1.e-2
        if     (.not.laxis(2)) then
*         write(6,'(a,i5)') 'laxis(2)==F for j=',j
*         do i= 1,idm_out
*           if     (abs(plon_out(i,1)-plon_out(i,j)).gt.1.e-2) then
*             write(6,'(a,2i5,2f10.4,1pe14.6)')
*    &          'i,j,lon,dif = ',i,j,plon_out(i,1),plon_out(i,j),
*    &                               plon_out(i,1)-plon_out(i,j)
*           endif
*         enddo !j
          exit
        endif
...
c     
     ...
     &                    dist_max )
      ...
      plat_max = maxval(plat_in(:,:)) + dist_max
      plat_min = minval(plat_in(:,:)) - dist_max
      write(6,'(a,3f9.3)')
     &  'dist_max,plat_min,max = ',dist_max,plat_min,plat_max
      call flush(6)
c
...
c     
      ...
      do jj= 1,jdm_out
        if     (laxis(2)) then
          if     (plat_out(1,jj).le.plat_min) then
            i_out(:,jj) = 1
            j_out(:,jj) = 1
            x_out(:,jj) = spval
            y_out(:,jj) = spval
            write(6,'(a,i5,2f9.2)')
     &        'jj,plat_out.1,plat_min = ',jj,plat_out(1,jj),plat_min
            call flush(6)
            cycle
          endif
          if     (plat_out(1,jj).ge.plat_max) then
            i_out(:,jj) = 1
            j_out(:,jj) = jdm
            x_out(:,jj) = spval
            y_out(:,jj) = spval
            write(6,'(a,i5,2f9.2)')
     &        'jj,plat_out.1,plat_max = ',jj,plat_out(1,jj),plat_max
            call flush(6)
            cycle
          endif
        else !not rectilinear
          if     (maxval(plat_out(:,jj)).le.plat_min) then
            i_out(:,jj) = 1
            j_out(:,jj) = 1
            x_out(:,jj) = spval
            y_out(:,jj) = spval
            write(6,'(a,i5,2f9.2)')
     &        'jj,plat_out.jj,plat_min = ',
     &         jj,maxval(plat_out(:,jj)),plat_min
            call flush(6)
            cycle
          endif
          if     (minval(plat_out(:,jj)).ge.plat_max) then
            i_out(:,jj) = 1
            j_out(:,jj) = jdm
            x_out(:,jj) = spval
            y_out(:,jj) = spval
            write(6,'(a,i5,2f9.2)')
     &        'jj,plat_out.jj,plat_max = ',
     &         jj,minval(plat_out(:,jj)),plat_max
            call flush(6)
            cycle
          endif
        endif !laxis(2):else
        if     (jj1.eq.0) then
          jj1 = jj
          write(6,'(a,i5)') 'jj1 = ',jj1
          call flush(6)
        endif
        do ii= 1,idm_out
          ldebug = mod(ii,      idm_out/10).eq.1 .and.
     &             mod(jj-jj1+1,jdm_out/10).eq.1
...
c
            ...
            if     (x_out(ii,jj).ne.spval .and.
     &              y_out(ii,jj).ne.spval      ) then 
              cycle
            endif
          endif
c
          if     (jj.eq.jj1) then
c           
c           find the first sigificant row by exhaustive search.
c
            if     (ii.eq.1) then
              ip = 1
              jp = 1
            elseif (x_out(ii-1,jj).ne.spval) then
              ip = i_out(ii-1,jj)
              jp = j_out(ii-1,jj)
            else
              ip = 1
...
c           
c           assume result is "close" to the nearest target point.
c
            if      (x_out(ii,jj-1).ne.spval) then
              ip = i_out(ii,jj-1)
              jp = j_out(ii,jj-1)
              if = ip-maxinc
              il = ip+maxinc
...
c
          ...
          inc  = 0
c
c         shortcut if outside latitude range
c
          if     (yp.le.plat_min .or.
     &            yp.ge.plat_max     ) then
            if     (ip.eq.idm) then
              i_out(ii,jj) = 1
            else
...
c           
          do jq= jf,jl
            if     (jq.lt.1) then
              cycle  !bottom boundary always closed
            endif
            if     (jq.gt.jdm) then
              j = jdm-1-(jq-jdm)  !assume an arctic patch
            else
              j =        jq
            endif
            if     (dist.eq.0.0) then
            ...
            do iq= if,il
              if     (iq.lt.1) then
                if     (.not.lperiod(1)) then
                  ...
                  i = idm + iq  !0 becomes idm
                endif
              elseif (iq.gt.idm) then
                if     (.not.lperiod(1)) then
                  cycle
                else
                  i = iq - idm  !idm+1 becomes 1
                endif
              else
                i = iq
              endif
              if     (jq.gt.jdm) then
                i = idm-mod(i-1,idm)  !assume an arctic patch
              endif
              ...
              if     (qdx*dx+dy.lt.dist) then
                ip   = i
                jp   = j
                inc  = max( abs(iq-(if+il)/2), abs(jq-(jf+jl)/2) )
                dist = qdx*dx+dy
                if     (ldebug) then
                  write(6,'(a,4i5,3f9.2)')
     &              'ii,jj,ip,jp,dx,dy,dist = ',
     &               ii,jj,ip,jp,dx,dy,dist
          ...
          enddo !jq
c
          if     (jl-jf+1.ne.jdm) then
            if     (inc.eq.maxinc) then  !maxinc possibly too small
              write(6,'(a,3i5)')
     &          'ii,jj,inc    = ',ii,jj,inc
              call flush(6)
            endif
            if     (inc.gt.incmax) then
              write(6,'(a,3i5)')
     &          'ii,jj,incmax = ',ii,jj,inc
              call flush(6)
            endif
...
c
      close(no)
      call zbiocl(no)
      end program isuba_gmapi

      subroutine blkdat(idm_out,jdm_out,maxinc,
     &                  chead, flnm_reg,flnm_map)
