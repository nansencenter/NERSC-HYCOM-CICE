...
c ---  from a sequence of HYCOM 2.0 archive files.
c
      character*80     flnm,flnm_f,flnm_l,flnm_b
c
...
c
      read (*,'(a)') flnm_f
      write (lp,'(2a)') 'first input file: ',flnm_f(1:len_trim(flnm_f))
      call flush(lp)
      read (*,'(a)') flnm_l
      write (lp,'(2a)') 'last  input file: ',flnm_l(1:len_trim(flnm_l))
      call flush(lp)
...
c
     ...
     &          status='new',action='write')
          write(nt,3000) iexpt,time
          write(lp,3000) iexpt,time
...
c --- on entry, flmn is of the form:  */archv_????_???_??.b
c --- on exit,  flmn is of the form:  */barov_????_???_??_(flmod).d
c
      integer l
c
      l = len_trim(flnm)
      flnm(l-18:l-14) = 'barov'
      flnm(l-1 :)     = '_' // flmod(1:len_trim(flmod)) // '.d'
      return
      end
