...
c --- 'hybthk' = HYBGEN: minimum thickness (m) for thin in thick-thin-thick
...
c
      ...
      call blkinr(hybthk,'hybthk','(a6," =",f10.4," m")')
...
c
      ...
      real,    parameter :: thinthk=0.5       !thin to thick layer ratio
c
...
c
c --- check for two thick layers around a significantly thiner layer
c
     ...
     &            th3d(i,j,k+1,n).gt.theta(i,j,k)+epsil      ) then
            ...
            if     (p(i,j,k+2)+hybthk.lt.p(i,j,kk+1) .and.
     &              dpmid.lt.hybthk                  .and.
     &              dpmid.lt.thinthk*min(dptop,dpbot)     ) then
c
