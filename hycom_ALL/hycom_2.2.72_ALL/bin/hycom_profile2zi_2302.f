      PROGRAM HYCOM_PROFILE2ZI
      IMPLICIT NONE
C
C  hycom_profile2zi - Usage: hycom_profile2zi archv.txt zi.txt archz.txt [itype [[nK] nT]]
C
C                 converts an HYCOM isopycnal text profile file to
C                 a finite volume fixed interface depth text profile file.
C
...
C                =3; piecewise parabolic method (PPM), WENO-like
...
C
C  For K-profiles, the input and output values are treated as cell 
C  averages with the cells extending between the layer centers above 
C  and below the interface where the K values are nominally located.
C
...
C
      ...
      CHARACTER*240 CARG
C
      CHARACTER*240 CFILEA,CFILEB,CFILEC,CFILED,CFORMAT
      CHARACTER*240 CLINE
      REAL          HYBISO,NOHISO,THK,DEPTH,DUM5(5),FLAG
      REAL          SIGISO(9999),PO(99999)
      REAL          SIT(104),SOT(104)
      INTEGER       IOS,ITYPE,K,KI,KK,KDM,KO,KP,NDIF,KT,NTRC
      INTEGER       I,KMAX
C
      REAL, ALLOCATABLE :: SI(:,:),SK(:,:),PI(:),ZI(:),PIK(:),
     &                     SO(:,:),OK(:,:),      ZO(:),POK(:)
C
      INTEGER       NSAMP
      REAL          PMAX
...
C
      ...
      ELSEIF (NARG.EQ.8) THEN  !undocumented option
        ...
        CALL GETARG(6,CLINE)
        READ(CLINE,*) NTRC
        CALL GETARG(7,CFILED)
        CALL GETARG(8,CLINE)
        READ(CLINE,*) HYBISO
      ELSE
        WRITE(6,"(2a)")
     +    'Usage: hycom_profile2zi archv.txt zi.txt archz.txt',
     +    ' [itype [[nK] nT]]'
        CALL EXIT(1)
      ENDIF
C
...
C
      ALLOCATE( PI(KDM+1), PIK(KDM+1), ZI(KDM),
     &          SI(KDM,5+NTRC), SK(KDM,3) )
      SK(:,:) = 0.0
C
      REWIND(11)
      DO K= 1,5
...
C
      ...
      CLOSE(12)
C
      ALLOCATE( ZO(KO), POK(KO+1), SO(KO,5+NTRC), OK(KO,3) )
C
      DO K= 1,KO
...
C
      FLAG = 9999.99
      IF     (ITYPE.EQ.0) THEN
        CALL LAYER2ZI_PCM( SI,PI,KDM,5+NTRC,
     &                     SO,PO,KO,        FLAG)
      ELSEIF (ITYPE.EQ.1) THEN
        CALL LAYER2ZI_PLM( SI,PI,KDM,5+NTRC,
     &                     SO,PO,KO,        FLAG, SIGISO,HYBISO)
      ELSEIF (ITYPE.EQ.2) THEN
        CALL LAYER2ZI_PPM( SI,PI,KDM,5+NTRC,
     &                     SO,PO,KO,        FLAG, SIGISO,HYBISO)
      ELSEIF (ITYPE.EQ.3) THEN
        CALL LAYER2ZI_WENO(SI,PI,KDM,5+NTRC,
     &                     SO,PO,KO,        FLAG, SIGISO,HYBISO)
      ELSE
        WRITE(6,"(2a)")
     +    'Usage: hycom_profile2zi archv.txt zi.txt archz.txt',
     +    ' [itype [[nK] nT]]'
        ...
        CALL EXIT(1)
      ENDIF
      NOHISO = -1.0  !always use hishest order remapping, ignore SIGISO
      IF     (NDIF.GT.0) THEN
        IF     (ITYPE.EQ.0) THEN
          CALL LAYER2ZI_PCM( SK,PIK,KDM,NDIF,
     &                       OK,POK,KO,      FLAG)
        ELSEIF (ITYPE.EQ.1) THEN
          CALL LAYER2ZI_PLM( SK,PIK,KDM,NDIF,
     &                       OK,POK,KO,      FLAG, SIGISO,NOHISO)
        ELSEIF (ITYPE.EQ.2) THEN
          CALL LAYER2ZI_PPM( SK,PIK,KDM,NDIF,
     &                       OK,POK,KO,      FLAG, SIGISO,NOHISO)
        ELSEIF (ITYPE.EQ.3) THEN
          CALL LAYER2ZI_WENO(SK,PIK,KDM,NDIF,
     &                       OK,POK,KO,      FLAG, SIGISO,NOHISO)
        ENDIF
      ENDIF
C
...
C
      ...
      WRITE(*,'(A,5e14.6)') ' INPUT TOTALS: ',SIT(1:5)
      WRITE(*,'(A,5e14.6)') 'OUTPUT TOTALS: ',SOT(1:5)
      IF     (NTRC.GT.0) THEN
        WRITE(*,'(/A)')       '      TRACERS:  1  2  3  4  5'
        WRITE(*,'(A,5e14.6)') ' INPUT TOTALS: ',SIT(6:5+MIN(NTRC,5))
        WRITE(*,'(A,5e14.6)') 'OUTPUT TOTALS: ',SOT(6:5+MIN(NTRC,5))
      ENDIF
      IF     (NTRC.GT.5) THEN
        WRITE(*,'(/A)')       '      TRACERS:  6  7  8  9 10'
        WRITE(*,'(A,5e14.6)') ' INPUT TOTALS: ',SIT(11:5+MIN(NTRC,10))
        WRITE(*,'(A,5e14.6)') 'OUTPUT TOTALS: ',SOT(11:5+MIN(NTRC,10))
      ENDIF
C
C     OUTPUT.
C
...
C
      DO K= 1,KO
        WRITE(21,CFORMAT)
     &    K,
     &    (SO(K,KK),KK=1,5     ),PO(K+1)-PO(K),ZO(K),
     &    (OK(K,KP),KP=1,  NDIF),
     &    (SO(K,KT),KT=6,5+NTRC)
