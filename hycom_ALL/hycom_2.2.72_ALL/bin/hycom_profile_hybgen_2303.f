...
C                 based on hybgen version 2.3.03
C
C   archv.txt    is assumed to be an HYCOM archive text profile file
C   blkdat.input is assumed to be an HYCOM parameter file (version 2.3.03)
...
C
      ...
      REAL          DP0K(999),DS0K(999),DEPTHS,ISOTOP,DPNS,DSNS
      REAL          THKBOT
c
      integer     mnproc,lp
...
C
     ...
     +            DP00I,DP0K,DS0K,DPNS,DSNS,DEPTHS,ISOTOP,THKBOT,
     +            LDEBUG)
C
      DO K= 1,KDM
        P(K+1) = P(K) + DP(K)/ONEM
...
C
      ...
      ENDDO
      WRITE(*,'(/A)')       '        LABEL:  T  S  R'
      WRITE(*,'(A,1p3e14.6)') ' INPUT TOTALS: ',SUMI(:)
      WRITE(*,'(A,1p3e14.6)') 'OUTPUT TOTALS: ',SUMO(:)
      WRITE(*,'(A,1p3e14.6)') 'OUT-IN TOTALS: ',SUMD(:)
      WRITE(*,*)
C
...
C
     ...
     &          '      thkns             dpth'
      DO K= 1,KDM
        THK = P(K+1) - P(K)
        WRITE(21,'(i4,2f8.2,3f8.3,1pg15.6,0pf13.6)')
...
c
      ...
      real    s1dsum( mxtrcr+4),   !sum of   scalar fields
     &        s1d(kdm,mxtrcr+4),   !original scalar fields
     &        f1d(kdm,mxtrcr+4),   !final    scalar fields
     &        c1d(kdm,mxtrcr+4,3), !interpolation coefficients
     &        dpi( kdm),           !original layer thicknesses, >= onezm
     ...
     &        delt,deltm,dels,delsm,q,qsum,qdep,qtr,qts,thkbop,
     &        zthk,oneum,onezm
...
c
      ...
      onemm = onem/1000.0
c
      onezm  = onem*1.e-20  !a zepto-metre, an effectively zero thickness
...
c
c --- identify the deepest layer kp with significant thickness (> oneum)
c
      kp = 2  !minimum allowed value
      do k=kk,3,-1
        if (p(i,j,k+1)-p(i,j,k).ge.oneum) then
          kp=k
          exit
        endif
      enddo
...
c
c --- massless or near-massless (thickness < oneum) layers
c --- conserve mass, but make them all the same as deepest significant layer
c
      qsum      = 0.0
      s1dsum(:) = 0.0
      do k=kp,kk
        q    = max( p(i,j,k+1)-p(i,j,k), 0.0 )  !enforce interface order
        qsum = qsum + q
        if     (hybflg.eq.0) then  !T&S
          s1dsum(1) = s1dsum(1) + temp(i,j,k,n)*q
          s1dsum(2) = s1dsum(2) + saln(i,j,k,n)*q
        elseif (hybflg.eq.1) then  !th&S
          s1dsum(1) = s1dsum(1) + th3d(i,j,k,n)*q
          s1dsum(2) = s1dsum(2) + saln(i,j,k,n)*q
        elseif (hybflg.eq.2) then  !th&T
          s1dsum(1) = s1dsum(1) + th3d(i,j,k,n)*q
          s1dsum(2) = s1dsum(2) + temp(i,j,k,n)*q
        endif
        do ktr= 1,ntracr
          s1dsum(2+ktr) = s1dsum(2+ktr) + tracer(i,j,k,n,ktr)*q
        enddo
        if (mxlmy) then
          s1dsum(ntracr+3) = s1dsum(ntracr+3) + q2( i,j,k,n)*q
          s1dsum(ntracr+4) = s1dsum(ntracr+4) + q2l(i,j,k,n)*q
        endif
      enddo !k
      q         = 1.0/qsum  !qsum must be > onemu and so > 0
      s1dsum(:) = s1dsum(:)*q
      do k=kp,kk
        if (k.le.nhybrd) then
c ---     homogenize near-bottom layers
          if     (hybflg.eq.0) then  !T&S
            temp(i,j,k,n)=s1dsum(1)
            saln(i,j,k,n)=s1dsum(2)
            th3d(i,j,k,n)=sig(s1dsum(1),s1dsum(2))-thbase
          elseif (hybflg.eq.1) then  !th&S
            th3d(i,j,k,n)=s1dsum(1)
            saln(i,j,k,n)=s1dsum(2)
            temp(i,j,k,n)=tofsig(s1dsum(1)+thbase,
     &                           s1dsum(2))
          elseif (hybflg.eq.2) then  !th&T
            th3d(i,j,k,n)=s1dsum(1)
            temp(i,j,k,n)=s1dsum(2)
            saln(i,j,k,n)=sofsig(s1dsum(1)+thbase,
     &                           s1dsum(2))
          endif
...
c ---       homogenize temp
        ...
        do ktr= 1,ntracr
          tracer(i,j,k,n,ktr)=s1dsum(2+ktr)
        enddo
        if (mxlmy) then
          q2 (i,j,k,n)=s1dsum(ntracr+3)
          q2l(i,j,k,n)=s1dsum(ntracr+4)
        endif
      enddo !k
c
...
c> June 2012 -- no Robert-Asselin filter on very first time step
c> June 2012 -- added btrmas Robert-Asselin filter
c> June 2012 -- conserve mass when filling near-massless near-bottom layers
