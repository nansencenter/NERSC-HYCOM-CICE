...
CKAL  PARAMETER (IWI=1440, JWI=720, KWI=33)
CKAL  PARAMETER (XFIN=0.125, YFIN=-89.875, DXIN=0.25, DYIN=0.25)
C
C     CLIM ARRAYS.
C
...
!KAL  REAL*4    TSEAIN(IWI,JWI),SSEAIN(IWI,JWI),RSEAIN(IWI,JWI),
!KAL +          ZLEV(KWI)
      REAL*4, ALLOCATABLE  :: TSEAIN(:,:),SSEAIN(:,:),RSEAIN(:,:)
      REAL*4               ::  ZLEV(500)
...
C
      INTEGER IBD(4)
!KAL  REAL*4  TSEAI(IWI+4,JWI+4),SSEAI(IWI+4,JWI+4),RSEAI(IWI+4,JWI+4)
!KAL  REAL*4  FXI(IWI+4),FYI(JWI+4),
!KAL +        WQSEA3(IWI+4,JWI+4,3),WK(3*(IWI+JWI+8)+1)
      REAL*4, ALLOCATABLE ::  TSEAI(:,:),SSEAI(:,:),RSEAI(:,:)
      REAL*4, ALLOCATABLE ::  FXI(:),FYI(:),WQSEA3(:,:,:),WK(:)
C
C     NAMELIST.
C
      INTEGER          JPR
...
C
      IF     (ICTYPE.EQ.1) THEN
        ...
        READ(IUNIT)
        CCNAME=' '
CKAL    Read on 1st pass
        READ(IUNIT) IWI,JWI,KWI,XFIN,YFIN,DXIN,DYIN,ZLEV(1:KWI)
CKAL    IF     (IWIT.NE.IWI .OR.
CKAL +          JWIT.NE.JWI .OR.
CKAL +          KWIT.NE.KWI .OR.
CKAL +          ABS(XFINT-XFIN).GT.0.01 .OR.
CKAL +          ABS(YFINT-YFIN).GT.0.01 .OR.
CKAL +          ABS(DXINT-DXIN).GT.0.01 .OR.
CKAL +          ABS(DYINT-DYIN).GT.0.01     ) THEN
          WRITE(6,9001) IUNIT,IWI,JWI,KWI,XFIN,YFIN,DXIN,DYIN
          CALL ZHFLSH(6)
CKAL      STOP
CKAL    ENDIF
        IUNIT=72
        ...
        READ(IUNIT)
        CCNAME=' '
        READ(IUNIT) IWIT,JWIT,KWIT,XFINT,YFINT,DXINT,DYINT,ZLEV(1:KWIT)
        IF     (IWIT.NE.IWI .OR.
     +          JWIT.NE.JWI .OR.
     +          KWIT.NE.KWI .OR.
     ...
     +          ABS(DYINT-DYIN).GT.0.01     ) THEN
          WRITE(6,9001) IUNIT,IWI,JWI,KWI,XFIN,YFIN,DXIN,DYIN
          WRITE(6,9000) IUNIT,IWIT,JWIT,KWIT,XFINT,YFINT,DXINT,DYINT
          CALL ZHFLSH(6)
          STOP
        ENDIF
...
!KAL  REAL*4  FXI(IWI+4),FYI(JWI+4),
!KAL +        WQSEA3(IWI+4,JWI+4,3),WK(3*(IWI+JWI+8)+1)
      allocate(TSEAIN(IWI,JWI),SSEAIN(IWI,JWI),RSEAIN(IWI,JWI))
      allocate(TSEAI(IWI+4,JWI+4),SSEAI(IWI+4,JWI+4),RSEAI(IWI+4,JWI+4))
      allocate(FXI(IWI+4),FYI(JWI+4))
      allocate(WQSEA3(IWI+4,JWI+4,3),WK(3*(IWI+JWI+8)+1))
      !top '(hit)'
C
