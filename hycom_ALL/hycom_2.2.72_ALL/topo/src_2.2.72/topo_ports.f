...
CKAL
     ...
     &   fatal_i, fatal_j, fatal_port, fatal_v
c
      character*13 fmt
...
cKAL    should suffice ...
        allocate(fatal_i((idm+2)*(jdm+2)))
        allocate(fatal_j((idm+2)*(jdm+2)))
        allocate(fatal_v((idm+2)*(jdm+2)))
        allocate(fatal_port((idm+2)*(jdm+2)))
        numfatal = 0
        lfatal = .false.
        do l= 1,nports
          lfatalp = .false.
...
c
     ...
     &                j.lt.1 .or. j.gt.jdm     ) then
                lfatalp = .true.
                numfatal=numfatal+1
                fatal_port(numfatal)=l
                fatal_i(numfatal)=i
                fatal_j(numfatal)=j
                fatal_v(numfatal)=-100
...
c
        if     (lfatal) then
          ...
          do i=1,numfatal
             write(6,
     &    '("Port ",i3, "(kdport=",i3,") :Point ",2i5, " Value ",i5)') 
     &          fatal_port(i),kdport(fatal_port(i)),
     &          fatal_i(i),fatal_j(i),fatal_v(i)
          end do
          write(6,*) "Value = -100 : outside grid"
          write(6,*) "Value =    9 : u or v cell is sea"
          write(6,*) "Value =    7 : land too close to port location"
          stop '(topo_ports)'
        endif
        end
