...
c
      logical ldouble,lshift,ldiscard,luniform
...
c ---              > 8.8 to    only double-up constant-sized tiles
c ---              > 9.0 to shift only        constant-sized tiles
c ---              > 9.8 to shift & double-up constant-sized tiles
...
c
      ...
      if     (luniform) then
        ...
        ldouble = sfudge.gt.9.8 .or. (sfudge.gt.8.8 .and. sfudge.lt.9.0)
      endif
c
...
c
        do j= 1,mpe
          ...
          IF (max(mpe,npe).ne.2 .and. .TRUE.  ) THEN  ! shrinkwrap
          j_s = jspt(j) - mbdy
          j_n = jspt(j) + jjpe(j)-1 + mbdy
          do i= 1-mbdy,idm+mbdy
            mapsum(i) = sum(map(i,j_s:j_n))
