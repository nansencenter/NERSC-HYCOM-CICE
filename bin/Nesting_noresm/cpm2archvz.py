...
def main(filemesh, merged_nc_file, bio_file=None, iexpt=1, iversn=22, yrflag=3):
    depth, plon, plat = read_mesh(filemesh)
    ip = (depth == 0)  # Land mask: True where depth is 0

    if not os.path.exists(merged_nc_file):
        raise FileNotFoundError(f"File not found: {merged_nc_file}")
    if bio_file is not None and not os.path.exists(bio_file):
        raise FileNotFoundError(f"Bio file not found: {bio_file}")

    logger.info(f"Processing unified merged file: {os.path.basename(merged_nc_file)}")
    
    with Dataset(merged_nc_file, "r") as ncid:
    ...
    if bio_file is None:
        ...
        header3 = "Physical and biogeochemical parameters\n"
...
if __name__ == "__main__":
    ...
    parser.add_argument('merged_nc_file', type=str, help="Target .merged_YYYY-MM.nc file")
    ...
    main(args.meshfile, args.merged_nc_file, bio_file=args.bio_file, iexpt=args.iexpt, iversn=args.iversn, yrflag=args.yrflag)
