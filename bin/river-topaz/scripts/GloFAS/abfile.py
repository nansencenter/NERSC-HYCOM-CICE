...
class ABFileRiverForcing(ABFile):
   """ Class for doing input/output on pairs of hycom .a and .b files. This is for forcing files"""
   fieldkeys = ["field", "min", "max"]
   ...
   def __init__(self, basename, action, mask=False, real4=True, endian="big", idm=None, jdm=None,
      ...
      super(ABFileRiverForcing, self).__init__(basename, action, mask=mask, real4=real4, endian=endian)
      self._cline1 = cline1
      self._cline2 = cline2
      if action == "w":
         pass
   ...
   def read_field_info(self):
      ...
      while line:
         m = re.match(
            "^(.*):dtime1,range[ ]*=[ ]+([0-9\-\.e+]+)[ ]+([0-9\-\.e+]+)[ ]*,[ ]*([0-9\-\.e+]+)[ ]*([0-9\-\.e+]+)",
            line)
         if m:
            self._fields[i] = {}
            self._fields[i]["field"] = m.group(1).strip()
...
class ABFileRiver(ABFile) :
   ...
   def read_field_info(self):
      ...
      while line:
         m = re.match(
            "^(.*): month,range[ ]*=[ ]+([0-9][0-9])+[ ]+([0-9\-\.E+]+)+[ ]+([0-9\-\.E+]*.)"
            ,line)
         if m:
            self._fields[i] = {}
            self._fields[i]["field"] = m.group(1).strip()
            self._fields[i]["month"] = float(m.group(2).strip())
   ...
   def read_field(self, field, month):
      """ Read field corresponding to fieldname and level from archive file"""
      elems = [(k, v["month"]) for k, v in self._fields.items() if v["field"] == field]
      dist = numpy.array([elem[1] - month for elem in elems])
      i = numpy.argmin(numpy.abs(dist))
      rec, dt = elems[i]
      w = self._filea.read_record(i)
      # print w
