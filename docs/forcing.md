...
High-frequency (monthly) river forcing uses GloFAS reanalysis data instead of the
AHYPE/EHYPE discharge climatology. It is generated by `bin/river_synoptic.sh`, which
`expt_preprocess.sh` can call automatically (online mode) or which you can run manually
(offline mode) to pre-generate a longer period.

Set `River_online=1` in `expt_preprocess.sh` to run it automatically. To run it manually,
set `River_online=0` in `expt_preprocess.sh` to prevent it from re-running during the job,
then call it from the experiment directory, passing the start and end of the period to generate
(same `YYYY-MM-DDTHH:MM:SS` format as `START`/`END` in `srjob.sh`):
...
cd $WORK/${CONFIGNAME}/expt_${EXPT_ID}
$HOME/NERSC-HYCOM-CICE/bin/river_synoptic.sh 2013-01-01T00:00:00 2014-01-01T00:00:00
```

Output is written to `$WORK/<CONFIGNAME>/force/rivers/<IEXPT>/riverh.[ab]`.

**Source data**

GloFAS input data and the TOPAZ runoff grid files are stored at:

```
/cluster/projects/nn9481k/GloFAS_data/data_v40
/cluster/projects/nn9481k/GloFAS_data/TOPAZrunoff_data/
```

The GloFAS source data and TOPAZ runoff grid files are read internally by
`river_synoptic.sh`; their paths are configured in
`bin/river-topaz/scripts/GloFAS/hycom_river_hifre.py`. You do not need to interact with
these files directly, unless the coverage period needs to be extended beyond 2024, in which
case the input dataset at `/cluster/projects/nn9481k/GloFAS_data/data_v40/` would need to
be updated.

:::{note}
The required model settings differ between HYCOM versions:

- **HYCOM 2.2.98**: set `priver=0` in `blkdat.input` and `highfq_river=.true.` in
  `hycom.opt`. The model reads `riverh.[ab]`.
- **HYCOM 2.3**: set `priver=2` in `blkdat.input`; `highfq_river` is no longer used.
  The model reads `rivers.[ab]`.
:::

For background on the method, see the technical documentation by Adrien Acchiardi
(`bin/river-topaz/river_forcing_documentation.pdf`, August 2025).
