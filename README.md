...
# Compiling HYCOM only from HYCOM-CICE

HYCOM model can be start from the HYCOM-CICE coupled model by setting the "iceflg" flag in the "blkdat.input" to zero. If "iceflg" is non-zero, the model compiles and links both HYCOM and CICE models to generate the hycom-cice executable. If "iceflg" is zero the hycom-cice executable is generated by only compiling the HYCOM model (note that we have changed name of executable from hycom to hycom-cice).
