...
if [ $# -ne 1 ] ; then
   echo "This script will set up files with the Levitus climatology. The "
...
# --- L is Glodap   directory (GLODAP_PATH)
#
D=$BASEDIR/relax/levitus/
S=$D/SCRATCH
L=$GLODAP_PATH
[  "${L}" == ""  ] && { echo "LEVITUS_PATH is not set " ; exit 1; }
[ ! -d $L ] && { echo "Cant find LEVITUS_PATH directory $L" ; exit 1; }
